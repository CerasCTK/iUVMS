...
Bootloader::Bootloader(bool sikRadio, QObject *parent) : QObject(parent), _sikRadio(sikRadio) {}
...
bool Bootloader::_binProgram(const FirmwareImage *image) {
    ...
    while (bytesSent < imageSize) {
        ...
        bool failed = true;
        if (_write(PROTO_PROG_MULTI) && _write((uint8_t)bytesToSend) && _write(imageBuf, bytesToSend) && _write(PROTO_EOC)) {
            if (_getCommandResponse()) {
                failed = false;
            }
        }
...
bool Bootloader::_ihxProgram(const FirmwareImage *image) {
    ...
    for (uint16_t index = 0; index < image->ihxBlockCount(); index++) {
        ...
        failed = true;
        if (_write(PROTO_LOAD_ADDRESS) && _write(flashAddress & 0xFF) && _write((flashAddress >> 8) & 0xFF) && _write(PROTO_EOC)) {
            _port.flush();
            if (_getCommandResponse()) {
                failed = false;
            }
        ...
        while (bytesLeftToWrite > 0) {
            ...
            failed = true;
            if (_write(PROTO_PROG_MULTI) && _write(bytesToWrite) && _write(&((uint8_t *)bytes.data())[bytesIndex], bytesToWrite) && _write(PROTO_EOC)) {
                _port.flush();
                if (_getCommandResponse()) {
                    failed = false;
                }
...
bool Bootloader::_ihxVerifyBytes(const FirmwareImage *image) {
    ...
    for (uint16_t index = 0; index < image->ihxBlockCount(); index++) {
        ...
        failed = true;
        if (_write(PROTO_LOAD_ADDRESS) && _write(readAddress & 0xFF) && _write((readAddress >> 8) & 0xFF) && _write(PROTO_EOC)) {
            _port.flush();
            if (_getCommandResponse()) {
                failed = false;
            }
        ...
        while (bytesLeftToRead > 0) {
            ...
            for (int i = 0; i < bytesToRead; i++) {
                if ((uint8_t)imageBytes[bytesIndex + i] != readBuf[i]) {
                    _errorString
                        = tr("Compare failed: expected(0x%1) actual(0x%2) at address: 0x%3").arg(imageBytes[bytesIndex + i], 2, 16, QLatin1Char('0')).arg(readBuf[i], 2, 16, QLatin1Char('0')).arg(readAddress + i, 8, 16, QLatin1Char('0'));
                    return false;
                }
            }
