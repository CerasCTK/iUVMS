...
bool FirmwareImage::_ihxLoad(const QString &ihxFilename) {
    ...
    while (true) {
        ...
        if (!_readByteFromStream(stream, blockByteCount) || !_readWordFromStream(stream, address) || !_readByteFromStream(stream, recordType) || !_readBytesFromStream(stream, blockByteCount, bytes) || !_readByteFromStream(stream, crc)) {
...
bool FirmwareImage::_px4Load(const QString &imageFilename) {
    ...
    bool success = _decompressJsonValue(
        px4Json,              // JSON object
...
bool FirmwareImage::_decompressJsonValue(
...
uint16_t FirmwareImage::ihxBlockCount(void) const { return _ihxBlocks.length(); }
