...
class JsonHelper {
  ...
  public:
    ...
    /// @return true: file is json, false: file is not json
    static bool isJsonFile(
        const QByteArray &bytes, ///< json bytes
        QJsonDocument &jsonDoc,  ///< returned json document
        QString &errorString
    ); ///< error on parse failure

    /// Saves the standard file header the json object
    static void saveQGCJsonFileHeader(
        QJsonObject &jsonObject, ///< root json object
    ...
    static bool loadPolygon(
        const QJsonArray &polygonArray, ///< Array of coordinates
    ...
    static void saveGeoCoordinateArray(
        const QVariantList &rgVarPoints, ///< points to save
    ...
    static void savePolygon(
        QmlObjectListModel &list, ///< List which contains vertices
  ...
  private:
    ...
    static bool _loadGeoCoordinate(const QJsonValue &jsonValue, bool altitudeRequired, QGeoCoordinate &coordinate, QString &errorString, bool geoJsonFormat);
