...
void Viewer3DTerrainGeometry::changeUpAxis(int from, int to) {
    ...
    for (std::size_t i = 0; i < _vertices.size(); i++) {
        ...
        vy = _vertices[i].y();
        vz = _vertices[i].z();
        _vertices[i] = QVector3D(tx[0] * vx + ty[0] * vy + tz[0] * vz, tx[1] * vx + ty[1] * vy + tz[1] * vz, tx[2] * vx + ty[2] * vy + tz[2] * vz);

        // transform normals
        nx = _normals[i].x();
        ny = _normals[i].y();
        nz = _normals[i].z();
        _normals[i] = QVector3D(tx[0] * nx + ty[0] * ny + tz[0] * nz, tx[1] * nx + ty[1] * ny + tz[1] * nz, tx[2] * nx + ty[2] * ny + tz[2] * nz);
    }
}
...
int Viewer3DTerrainGeometry::sectorCount() const { return _sectorCount; }
...
int Viewer3DTerrainGeometry::stackCount() const { return _stackCount; }
    ...
    for (i = 0; i < _stackCount; ++i) {
        ...
        for (j = 0; j < _sectorCount; ++j, ++vi1, ++vi2) {
            ...
            if (stackAngle < 90) {
                ...
                _n = computeFaceNormal(QVector3D(v1.x, v1.y, v1.z), QVector3D(v2.x, v2.y, v2.z), QVector3D(v3.x, v3.y, v3.z));
            ...
            if (stackAngle > -90) {
                ...
                _n = computeFaceNormal(QVector3D(v3.x, v3.y, v3.z), QVector3D(v2.x, v2.y, v2.z), QVector3D(v4.x, v4.y, v4.z));
...
int Viewer3DTerrainGeometry::radius() const { return _radius; }
...
QGeoCoordinate Viewer3DTerrainGeometry::roiMin() const { return _roiMin; }
...
QGeoCoordinate Viewer3DTerrainGeometry::roiMax() const { return _roiMax; }
...
QGeoCoordinate Viewer3DTerrainGeometry::refCoordinate() const { return _refCoordinate; }
