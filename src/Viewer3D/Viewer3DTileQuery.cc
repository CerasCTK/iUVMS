...
MapTileQuery::MapTileQuery(QObject *parent) : QObject{ parent } {}
...
MapTileQuery::TileStatistics_t MapTileQuery::findAndLoadMapTiles(int zoomLevel, QGeoCoordinate coordinate_1, QGeoCoordinate coordinate_2) {
    float lat_1 = coordinate_1.latitude();
    float lon_1 = coordinate_1.longitude();
    float lat_2 = coordinate_2.latitude();
    float lon_2 = coordinate_2.longitude();
...
double MapTileQuery::valueClip(double n, double _minValue, double _maxValue) { return fmin(fmax(n, _minValue), _maxValue); }
...
QPoint MapTileQuery::pixelXYToTileXY(QPoint pixel) { return QPoint(pixel.x() / 256, pixel.y() / 256); }

QPoint MapTileQuery::tileXYToPixelXY(QPoint tile) { return QPoint(tile.x() * 256, tile.y() * 256); }
...
QString MapTileQuery::getTileKey(int mapId, int x, int y, int zoomLevel) { return QString::asprintf("%010d%08d%08d%03d", mapId, x, y, zoomLevel); }
