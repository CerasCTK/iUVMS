...
JoystickManager::JoystickManager(QObject *parent) : QObject(parent), _joystickCheckTimer(new QTimer(this)) {
...
JoystickManager *JoystickManager::instance() { return _joystickManager(); }
...
Joystick *JoystickManager::activeJoystick() { return _activeJoystick; }
...
QString JoystickManager::activeJoystickName() const { return (_activeJoystick ? _activeJoystick->name() : QString()); }
...
void JoystickManager::_updateAvailableJoysticks() {
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            case SDL_QUIT: qCDebug(JoystickManagerLog) << "SDL ERROR:" << SDL_GetError(); break;
