...
GPSProvider::GPSProvider(const QString &device, GPSType type, const rtk_data_s &rtkData, const std::atomic_bool &requestStop, QObject *parent) : QThread(parent), _device(device), _type(type), _requestStop(requestStop), _rtkData(rtkData) {
...
int GPSProvider::callback(GPSCallbackType type, void *data1, int data2) {
    switch (type) {
        ...
        case GPSCallbackType::writeDeviceData:
            ...
            return -1;
        case GPSCallbackType::setBaudrate: return (_serial->setBaudRate(data2) ? 0 : -1);
        case GPSCallbackType::gotRTCMMessage: _gotRTCMData(reinterpret_cast<uint8_t *>(data1), data2); break;
