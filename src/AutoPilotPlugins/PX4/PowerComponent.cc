...
PowerComponent::PowerComponent(Vehicle *vehicle, AutoPilotPlugin *autopilot, QObject *parent) : VehicleComponent(vehicle, autopilot, parent), _name(tr("Power")) {}

QString PowerComponent::name(void) const { return _name; }

QString PowerComponent::description(void) const { return tr("Power Setup is used to setup battery parameters as well as advanced settings for propellers."); }
...
bool PowerComponent::setupComplete(void) const {
    if (!_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "BAT1_SOURCE") || !_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "BAT1_V_CHARGED")
        || !_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "BAT1_V_EMPTY") || !_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "BAT1_N_CELLS")) {
        return true;
    }
...
QStringList PowerComponent::setupCompleteChangedTriggerList(void) const { return { "BAT1_SOURCE", "BAT1_V_CHARGED", "BAT1_V_EMPTY", "BAT1_N_CELLS" }; }
