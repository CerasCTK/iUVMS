...
PX4AutoPilotPlugin::PX4AutoPilotPlugin(Vehicle *vehicle, QObject *parent)
    : AutoPilotPlugin(vehicle, parent), _incorrectParameterVersion(false), _airframeComponent(nullptr), _radioComponent(nullptr), _esp8266Component(nullptr), _flightModesComponent(nullptr), _sensorsComponent(nullptr),
      _safetyComponent(nullptr), _cameraComponent(nullptr), _powerComponent(nullptr), _motorComponent(nullptr), _actuatorComponent(nullptr), _tuningComponent(nullptr), _flightBehavior(nullptr), _syslinkComponent(nullptr) {
    if (!vehicle) {
...
void PX4AutoPilotPlugin::parametersReadyPreChecks(void) {
    ...
    if (_vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, hitlParam) && _vehicle->parameterManager()->getParameter(ParameterManager::defaultComponentId, hitlParam)->rawValue().toBool()) {
