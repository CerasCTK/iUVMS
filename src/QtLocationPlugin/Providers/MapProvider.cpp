...
MapProvider::MapProvider(const QString &mapName, const QString &referrer, const QString &imageFormat, quint32 averageSize, QGeoMapType::MapStyle mapStyle)
    : _mapName(mapName), _referrer(referrer), _imageFormat(imageFormat), _averageSize(averageSize), _mapStyle(mapStyle), _language(!QLocale::system().uiLanguages().isEmpty() ? QLocale::system().uiLanguages().constFirst() : "en"),
      _mapId(_mapIdIndex++) {
...
QUrl MapProvider::getTileURL(int x, int y, int zoom) const { return QUrl(_getURL(x, y, zoom)); }
...
int MapProvider::long2tileX(double lon, int z) const { return static_cast<int>(floor((lon + 180.0) / 360.0 * pow(2.0, z))); }

int MapProvider::lat2tileY(double lat, int z) const { return static_cast<int>(floor((1.0 - log(tan(lat * M_PI / 180.0) + 1.0 / cos(lat * M_PI / 180.0)) / M_PI) / 2.0 * pow(2.0, z))); }
