...
ChannelConfigInstance *ChannelConfig::instantiate(int paramIndex, int actuatorTypeIndex, ParameterManager *parameterManager, std::function<void(Function, Fact *)> factAddedCb) {
...
ChannelConfigVirtualAxis::instantiate([[maybe_unused]] int paramIndex, [[maybe_unused]] int actuatorTypeIndex, [[maybe_unused]] ParameterManager *parameterManager, [[maybe_unused]] std::function<void(Function, Fact *)> factAddedCb) {
...
void ChannelConfigInstanceVirtualAxis::allInstancesInitialized(QmlObjectListModel *configInstances) {
    ...
    // Initialize fact
    QStringList enumStrings{ tr("Custom"), tr("Upwards"), tr("Downwards"), tr("Forwards"), tr("Backwards"), tr("Leftwards"), tr("Rightwards") };
    ...
    for (int i = 0; i < 3; ++i) {
        connect(_axes[i]->fact(), &Fact::rawValueChanged, this, [this]() { ChannelConfigInstanceVirtualAxis::setFactFromAxes(true); });
    }
    // Inherit visibility & enabled from the first axis
    connect(_axes[0], &ChannelConfigInstance::visibleChanged, this, &ChannelConfigInstanceVirtualAxis::axisVisibleChanged);
    connect(_axes[0], &ChannelConfigInstance::enabledChanged, this, &ChannelConfigInstanceVirtualAxis::axisEnableChanged);
    axisVisibleChanged();
    axisEnableChanged();
}
...
MixerChannel::MixerChannel(
    QObject *parent, const QString &label, int actuatorFunction, int paramIndex, int actuatorTypeIndex, QmlObjectListModel &channelConfigs, ParameterManager *parameterManager, const Rule *rule,
    std::function<void(Function, Fact *)> factAddedCb
)
    : QObject(parent), _label(label), _actuatorFunction(actuatorFunction), _paramIndex(paramIndex), _actuatorTypeIndex(actuatorTypeIndex), _rule(rule) {
...
bool MixerChannel::getGeometry(const ActuatorTypes &actuatorTypes, const MixerOption::ActuatorGroup &group, ActuatorGeometry &geometry) const {
    ...
    for (int i = 0; i < _configInstances->count(); ++i) {
        ...
        switch (configInstance->channelConfig()->function()) {
            ...
            case Function::PositionZ:
                ...
                ++numPositionAxis;
                break;
            case Function::SpinDirection: geometry.spinDirection = configInstance->fact()->rawValue().toBool() ? ActuatorGeometry::SpinDirection::CounterClockWise : ActuatorGeometry::SpinDirection::ClockWise; break;
            case Function::AxisX:
            case Function::AxisY:
            case Function::AxisZ:
            case Function::Type:
            case Function::Unspecified: break;
        }
    }
...
void Mixers::reset(const ActuatorTypes &actuatorTypes, const MixerOptions &mixerOptions, const QMap<int, OutputFunction> &functions, const Rules &rules) {
...
void Mixers::update() {
    ...
    if (_selectedMixer != -1) {
        ...
        for (const auto &actuatorGroup : actuatorGroups) {
            ...
            // 'count' param
            if (actuatorGroup.count != "") {
                currentMixerGroup->setCountParam(new ConfigParameter(currentMixerGroup, getFact(actuatorGroup.count), "", false));
            }
            ...
            for (int actuatorIdx = 0; actuatorIdx < count; ++actuatorIdx) {
                ...
                MixerChannel *channel = new MixerChannel(currentMixerGroup, label, actuatorFunction, actuatorIdx, actuatorTypeIndex, *currentMixerGroup->channelConfigs(), _parameterManager, selectedRule, factAdded);
            ...
            for (const auto &parameter : actuatorGroup.parameters) {
                currentMixerGroup->addConfigParam(new ConfigParameter(currentMixerGroup, getFact(parameter.name), parameter.label, parameter.advanced));
            }
