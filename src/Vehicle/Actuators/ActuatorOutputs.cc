...
ActuatorOutputChannel::ActuatorOutputChannel(QObject *parent, const QString &label, int paramIndex, QmlObjectListModel &channelConfigs, ParameterManager *parameterManager, std::function<void(Fact *)> factAddedCb)
...
ActuatorOutput::ActuatorOutput(QObject *parent, const QString &label, const Condition &groupVisibilityCondition) : QObject(parent), _label(label), _groupVisibilityCondition(groupVisibilityCondition) {
...
void ActuatorOutput::getAllChannelFunctions(QList<Fact *> &allFunctions) const {
    forEachOutputFunction([&allFunctions](ActuatorOutputSubgroup *, ChannelConfigInstance *, Fact *fact) { allFunctions.append(fact); });
}

bool ActuatorOutput::hasExistingOutputFunctionParams() const {
    bool hasExistingOutputFunction = false;
    forEachOutputFunction([&hasExistingOutputFunction](ActuatorOutputSubgroup *, ChannelConfigInstance *, [[maybe_unused]] Fact *fact) { hasExistingOutputFunction = true; });
