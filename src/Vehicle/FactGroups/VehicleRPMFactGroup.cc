...
VehicleRPMFactGroup::VehicleRPMFactGroup(QObject *parent)
    : FactGroup(1000, ":/json/Vehicle/RPMFact.json", parent), _rpm1Fact(0, _rpm1FactName, FactMetaData::valueTypeDouble), _rpm2Fact(0, _rpm2FactName, FactMetaData::valueTypeDouble),
      _rpm3Fact(0, _rpm3FactName, FactMetaData::valueTypeDouble), _rpm4Fact(0, _rpm4FactName, FactMetaData::valueTypeDouble) {
...
void VehicleRPMFactGroup::handleMessage(Vehicle * /* vehicle */, mavlink_message_t &message) {
    if (message.msgid == MAVLINK_MSG_ID_RAW_RPM) {
        ...
        switch (raw_rpm.index) {
            case 0: rpm1()->setRawValue(raw_rpm.frequency); break;
            case 1: rpm2()->setRawValue(raw_rpm.frequency); break;
            case 2: rpm3()->setRawValue(raw_rpm.frequency); break;
            case 3: rpm4()->setRawValue(raw_rpm.frequency); break;
            default: break;
        }
