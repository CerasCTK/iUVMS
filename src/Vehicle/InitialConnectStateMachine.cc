...
InitialConnectStateMachine::InitialConnectStateMachine(Vehicle *vehicle, QObject *parent) : StateMachine(parent), _vehicle(vehicle) {
...
void InitialConnectStateMachine::gotProgressUpdate(float progressValue) { emit progressUpdate(_progress(progressValue)); }
...
void InitialConnectStateMachine::_stateRequestAutopilotVersion(StateMachine *stateMachine) {
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isLogReplay()) {
            ...
            vehicle->requestMessage(_autopilotVersionRequestMessageHandler, connectMachine, MAV_COMP_ID_AUTOPILOT1, MAVLINK_MSG_ID_AUTOPILOT_VERSION);
        }
    }
}
...
void InitialConnectStateMachine::_autopilotVersionRequestMessageHandler(void *resultHandlerData, MAV_RESULT commandResult, Vehicle::RequestMessageResultHandlerFailureCode_t failureCode, const mavlink_message_t &message) {
    ...
    switch (failureCode) {
        ...
        case Vehicle::RequestMessageFailureCommandError: qCDebug(InitialConnectStateMachineLog) << QStringLiteral("REQUEST_MESSAGE:AUTOPILOT_VERSION command error(%1)").arg(commandResult); break;
        case Vehicle::RequestMessageFailureCommandNotAcked: qCDebug(InitialConnectStateMachineLog) << "REQUEST_MESSAGE:AUTOPILOT_VERSION command never acked"; break;
        case Vehicle::RequestMessageFailureMessageNotReceived: qCDebug(InitialConnectStateMachineLog) << "REQUEST_MESSAGE:AUTOPILOT_VERSION command acked but message never received"; break;
        case Vehicle::RequestMessageFailureDuplicateCommand: qCDebug(InitialConnectStateMachineLog) << "REQUEST_MESSAGE:AUTOPILOT_VERSION Internal Error: Duplicate command"; break;
    }
...
void InitialConnectStateMachine::_stateRequestProtocolVersion(StateMachine *stateMachine) {
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isLogReplay()) {
            ...
            vehicle->requestMessage(_protocolVersionRequestMessageHandler, connectMachine, MAV_COMP_ID_AUTOPILOT1, MAVLINK_MSG_ID_PROTOCOL_VERSION);
        }
    }
}
...
void InitialConnectStateMachine::_protocolVersionRequestMessageHandler(void *resultHandlerData, MAV_RESULT commandResult, Vehicle::RequestMessageResultHandlerFailureCode_t failureCode, const mavlink_message_t &message) {
    ...
    switch (failureCode) {
        ...
        case Vehicle::RequestMessageFailureCommandError: qCDebug(InitialConnectStateMachineLog) << QStringLiteral("REQUEST_MESSAGE PROTOCOL_VERSION command error(%1)").arg(commandResult); break;
        case Vehicle::RequestMessageFailureCommandNotAcked: qCDebug(InitialConnectStateMachineLog) << "REQUEST_MESSAGE PROTOCOL_VERSION command never acked"; break;
        case Vehicle::RequestMessageFailureMessageNotReceived: qCDebug(InitialConnectStateMachineLog) << "REQUEST_MESSAGE PROTOCOL_VERSION command acked but message never received"; break;
        case Vehicle::RequestMessageFailureDuplicateCommand: qCDebug(InitialConnectStateMachineLog) << "REQUEST_MESSAGE PROTOCOL_VERSION Internal Error: Duplicate command"; break;
    }
...
void InitialConnectStateMachine::_stateRequestCompInfo(StateMachine *stateMachine) {
    ...
    connect(vehicle->_componentInformationManager, &ComponentInformationManager::progressUpdate, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
...
void InitialConnectStateMachine::_stateRequestStandardModes(StateMachine *stateMachine) {
    ...
    connect(vehicle->_standardModes, &StandardModes::requestCompleted, connectMachine, &InitialConnectStateMachine::standardModesRequestCompleted);
    vehicle->_standardModes->request();
}

void InitialConnectStateMachine::standardModesRequestCompleted() {
    disconnect(_vehicle->_standardModes, &StandardModes::requestCompleted, this, &InitialConnectStateMachine::standardModesRequestCompleted);
    advance();
}
...
void InitialConnectStateMachine::_stateRequestCompInfoComplete(void *requestAllCompleteFnData) {
    ...
    disconnect(connectMachine->_vehicle->_componentInformationManager, &ComponentInformationManager::progressUpdate, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
...
void InitialConnectStateMachine::_stateRequestParameters(StateMachine *stateMachine) {
    ...
    connect(vehicle->_parameterManager, &ParameterManager::loadProgressChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
...
void InitialConnectStateMachine::_stateRequestMission(StateMachine *stateMachine) {
    ...
    disconnect(vehicle->_parameterManager, &ParameterManager::loadProgressChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isLogReplay()) {
            ...
            connect(vehicle->_missionManager, &MissionManager::progressPctChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
        }
    }
}
...
void InitialConnectStateMachine::_stateRequestGeoFence(StateMachine *stateMachine) {
    ...
    disconnect(vehicle->_missionManager, &MissionManager::progressPctChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isLogReplay()) {
            ...
            if (vehicle->_geoFenceManager->supported()) {
                ...
                connect(vehicle->_geoFenceManager, &GeoFenceManager::progressPctChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
            } else {
...
void InitialConnectStateMachine::_stateRequestRallyPoints(StateMachine *stateMachine) {
    ...
    disconnect(vehicle->_geoFenceManager, &GeoFenceManager::progressPctChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isLogReplay()) {
            ...
            if (vehicle->_rallyPointManager->supported()) {
                ...
                connect(vehicle->_rallyPointManager, &RallyPointManager::progressPctChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
            } else {
...
void InitialConnectStateMachine::_stateSignalInitialConnectComplete(StateMachine *stateMachine) {
    ...
    disconnect(vehicle->_rallyPointManager, &RallyPointManager::progressPctChanged, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
