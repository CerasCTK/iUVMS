...
QGCApplication::QGCApplication(int &argc, char *argv[], bool unitTesting)
    ...
    connect(
        &_missingParamsDelayedDisplayTimer, &QTimer::timeout, this,
    ...
    if (_runningUnitTests) {
        // We don't want unit tests to use the same QSettings space as the
        // normal app. So we tweak the app name. Also we want to run unit tests
        // with clean settings every time.
        applicationName = QStringLiteral("%1_unittest").arg(UVMS_APP_NAME);
    } else {
#ifdef DAILY_BUILD
        // This gives daily builds their own separate settings space. Allowing
        // you to use daily and stable builds side by side without daily
        // screwing up your stable settings.
    ...
    if (fClearSettingsOptions) {
        ...
        // Check and clear must happen before toolbox is started since that will
        // write some settings.
...
void QGCApplication::setLanguage() {
    ...
    qCDebug(QGCApplicationLog)
        << "System reported locale:" << _locale << "; Name" << _locale.name()
        << "; Preffered (used in maps): "
        << (QLocale::system().uiLanguages().length() > 0
                ? QLocale::system().uiLanguages()[0]
                : "None");

    QLocale::Language possibleLocale
        = AppSettings::_qLocaleLanguageEarlyAccess();
    ...
    if (_locale == QLocale::Korean) {
        ...
        if (QFontDatabase::addApplicationFont(":/fonts/NanumGothic-Regular")
            < 0) {
    ...
    if (_locale.name() != "en_US") {
        ...
        if (_qgcTranslatorQtLibs.load(
            ...
            qCWarning(LocalizationLog) << "Qt lib localization for"
                                       << _locale.name() << "is not present";
        }
        if (_qgcTranslatorSourceCode.load(
                _locale, QLatin1String("qgc_source_"), "", ":/i18n"
            )) {
        ...
        if (JsonHelper::translator()->load(
                _locale, QLatin1String("qgc_json_"), "", ":/i18n"
            )) {
...
void QGCApplication::init() {
    ...
    qmlRegisterUncreatableType<GimbalController>(
    ...
    qmlRegisterType<MAVLinkInspectorController>(
        "QGroundControl.Controllers", 1, 0, "MAVLinkInspectorController"
    );
#endif
    qmlRegisterType<GeoTagController>(
        "QGroundControl.Controllers", 1, 0, "GeoTagController"
    );
    qmlRegisterType<LogDownloadController>(
        "QGroundControl.Controllers", 1, 0, "LogDownloadController"
    );
    qmlRegisterType<MAVLinkConsoleController>(
        "QGroundControl.Controllers", 1, 0, "MAVLinkConsoleController"
    );

    qmlRegisterUncreatableType<AutoPilotPlugin>(
        "QGroundControl.AutoPilotPlugin", 1, 0, "AutoPilotPlugin",
        "Reference only"
    );
    qmlRegisterType<ESP8266ComponentController>(
        "QGroundControl.Controllers", 1, 0, "ESP8266ComponentController"
    );
    qmlRegisterType<SyslinkComponentController>(
        "QGroundControl.Controllers", 1, 0, "SyslinkComponentController"
    );

    qmlRegisterUncreatableType<VehicleComponent>(
        "QGroundControl.AutoPilotPlugin", 1, 0, "VehicleComponent",
        "Reference only"
    );
#ifndef NO_SERIAL_LINK
    qmlRegisterType<FirmwareUpgradeController>(
        "QGroundControl.Controllers", 1, 0, "FirmwareUpgradeController"
    );
#endif
    qmlRegisterType<JoystickConfigController>(
        "QGroundControl.Controllers", 1, 0, "JoystickConfigController"
    );
    ...
    qmlRegisterSingletonType<QGCMAVLink>(
        "MAVLink", 1, 0, "MAVLink", mavlinkSingletonFactory
    );
...
void QGCApplication::_initForNormalAppBoot() {
    ...
    ); // CorePlugin must be initialized before VideoManager for Video Cleanup
    ...
    QObject::connect(
        _qmlAppEngine, &QQmlApplicationEngine::objectCreationFailed, this,
...
#ifndef NO_SERIAL_LINK
    if (!_runningUnitTests) {
        // Determine if we have the correct permissions to access USB serial
        // devices
        QFile permFile("/etc/group");
        if (permFile.open(QIODevice::ReadOnly)) {
            while (!permFile.atEnd()) {
                const QString line = permFile.readLine();
                if (line.contains("dialout")
                    && !line.contains(getenv("USER"))) {
                    permFile.close();
                    showAppMessage(QString(
                        tr("The current user does not have the correct "
...
void QGCApplication::warningMessageBoxOnMainThread(
    const QString & /*title*/, const QString &msg
) {
    showAppMessage(msg);
}

void QGCApplication::criticalMessageBoxOnMainThread(
    const QString & /*title*/, const QString &msg
) {
    showAppMessage(msg);
}

void QGCApplication::reportMissingParameter(
...
void QGCApplication::_missingParamsDisplay(void) {
    if (_missingParams.count()) {
        ...
        showAppMessage(
            tr("Parameters are missing from firmware. You may be running a "
               "version of firmware which is not fully supported or your "
               "firmware has a bug in it. Missing params: %1")
                .arg(params)
        );
    }
}
...
void QGCApplication::showCriticalVehicleMessage(const QString &message) {
    ...
    if (message.startsWith(QStringLiteral("PreArm"))
        || message.startsWith(
            QStringLiteral("preflight"), Qt::CaseInsensitive
        )) {
        return;
    }
    ...
    if (rootQmlObject && _showErrorsInToolbar) {
        ...
        QMetaObject::invokeMethod(
            rootQmlObject, "showCriticalVehicleMessage",
            Q_RETURN_ARG(QVariant, varReturn), Q_ARG(QVariant, varMessage)
        );
...
void QGCApplication::showAppMessage(
    ...
    if (rootQmlObject) {
        ...
        QVariant varMessage = QVariant::fromValue(message);
        QMetaObject::invokeMethod(
            _rootQmlObject(), "_showMessageDialog",
            Q_RETURN_ARG(QVariant, varReturn), Q_ARG(QVariant, dialogTitle),
            Q_ARG(QVariant, varMessage)
        );
    } else if (runningUnitTests()) {
        // Unit tests can run without UI
        qCDebug(QGCApplicationLog)
            << "QGCApplication::showAppMessage unittest title:message"
            << dialogTitle << message;
    } else {
        // UI isn't ready yet
...
void QGCApplication::showRebootAppMessage(
    const QString &message, const QString &title
) {
...
void QGCApplication::showSetupView() {
    if (_rootQmlObject()) {
        QVariant arg = "";
        QMetaObject::invokeMethod(
            _rootQmlObject(), "showVehicleSetupTool", Q_ARG(QVariant, arg)
        );
    }
}
...
void QGCApplication::_checkForNewVersion() {
    if (!_runningUnitTests) {
        if (_parseVersionText(
                applicationVersion(), _majorVersion, _minorVersion,
                _buildVersion
            )) {
            ...
            if (!versionCheckFile.isEmpty()) {
                ...
                connect(
                    download, &QGCFileDownload::downloadComplete, this,
        ...
        if (versionFile.open(QIODevice::ReadOnly)) {
            ...
            if (_parseVersionText(
                ...
                if (_majorVersion < majorVersion
                    ...
                    showAppMessage(
                        tr("There is a newer version of %1 available. You can "
                           "download it from %2.")
                            .arg(applicationName())
                            .arg(QGCCorePlugin::instance()
...
int QGCApplication::CompressedSignalList::_signalIndex(const QMetaMethod &method
    ...
    if (method.methodType() != QMetaMethod::Signal) {
        qCWarning(QGCApplicationLog)
            << "Internal error: "
               "QGCApplication::CompressedSignalList::_signalIndex not a signal"
            << method.methodType();
        return -1;
    }
...
bool QGCApplication::compressEvent(
    ...
    for (QPostEventList::iterator it = postedEvents->begin();
        ...
        const QMetaCallEvent *cur_mce
            = static_cast<QMetaCallEvent *>(cur.event);
        ...
        // We can't merely qSwap the existing posted event with the new one,
        // since QEvent keeps track of whether it has been posted. Deletion of a
        // formerly posted event takes the posted event list mutex and does a
        // useless search of the posted event list upon deletion. We thus clear
        // the QEvent::posted flag before deletion.
...
bool QGCApplication::event(QEvent *e) {
    if (e->type() == QEvent::Quit) {
        ...
        // ApplicationWindow does not signal closing. Instead you get a Quit
        // event here only. This in turn causes the standard QGC shutdown
        // sequence to not run. So in this case we close the window ourselves
        // such that the signal is sent and the normal shutdown sequence runs.
        const bool forceClose
            = _mainRootWindow->property("_forceClose").toBool();
        ...
        //  shutdown sequence has not been run yet. Don't let this event close
        //  the app yet. Close the main window to kick off the normal shutdown.
        if (!forceClose) {
            //
...
QGCImageProvider *QGCApplication::qgcImageProvider() {
    return dynamic_cast<QGCImageProvider *>(
        _qmlAppEngine->imageProvider(qgcImageProviderId)
    );
}
...
QString QGCApplication::bigSizeToString(quint64 size) {
    ...
    if (size < 1024) {
        ...
        result = kLocale.toString(static_cast<double>(size) / 1024.0, 'f', 1)
                 + "KB";
    } else if (size < pow(1024, 3)) {
        result
            = kLocale.toString(static_cast<double>(size) / pow(1024, 2), 'f', 1)
              + "MB";
    } else if (size < pow(1024, 4)) {
        result
            = kLocale.toString(static_cast<double>(size) / pow(1024, 3), 'f', 1)
              + "GB";
    } else {
        result
            = kLocale.toString(static_cast<double>(size) / pow(1024, 4), 'f', 1)
              + "TB";
    }
    return result;
}
...
QString QGCApplication::bigSizeMBToString(quint64 size_MB) {
    ...
    if (size_MB < 1024) {
        ...
        result = kLocale.toString(static_cast<double>(size_MB) / 1024.0, 'f', 1)
                 + " GB";
    } else {
        result = kLocale.toString(
                     static_cast<double>(size_MB) / pow(1024, 2), 'f', 2
                 )
                 + " TB";
    }
    return result;
