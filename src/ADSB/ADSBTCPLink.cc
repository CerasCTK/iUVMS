...
ADSBTCPLink::ADSBTCPLink(const QHostAddress &hostAddress, quint16 port, QObject *parent) : QObject(parent), _hostAddress(hostAddress), _port(port), _socket(new QTcpSocket(this)), _processTimer(new QTimer(this)) {
#ifdef QT_DEBUG
    (void)connect(
        _socket, &QTcpSocket::stateChanged, this,
        [](QTcpSocket::SocketState state) {
            switch (state) {
                case QTcpSocket::UnconnectedState: qCDebug(ADSBTCPLinkLog) << "ADSB Socket disconnected"; break;
                case QTcpSocket::SocketState::ConnectingState: qCDebug(ADSBTCPLinkLog) << "ADSB Socket connecting..."; break;
                case QTcpSocket::SocketState::ConnectedState: qCDebug(ADSBTCPLinkLog) << "ADSB Socket connected"; break;
                case QTcpSocket::SocketState::ClosingState: qCDebug(ADSBTCPLinkLog) << "ADSB Socket closing..."; break;
                default: break;
            }
        },
        Qt::AutoConnection
    ...
    (void)QObject::connect(
        _socket, &QTcpSocket::errorOccurred, this,
        [this](QTcpSocket::SocketError error) {
...
void ADSBTCPLink::_parseLine(const QString &line) {
    ...
    switch (msgType) {
        ...
        case ADSB::SurveillanceId: _parseAndEmitCallsign(adsbInfo, values); break;
        case ADSB::AirbornePosition: _parseAndEmitLocation(adsbInfo, values); break;
        case ADSB::AirborneVelocity: _parseAndEmitHeading(adsbInfo, values); break;
        default: break;
    }
}
