...
PX4FirmwarePluginInstanceData::PX4FirmwarePluginInstanceData(QObject *parent) : QObject(parent), versionNotified(false) {}

PX4FirmwarePlugin::PX4FirmwarePlugin()
    : _manualFlightMode(tr("Manual")), _acroFlightMode(tr("Acro")), _stabilizedFlightMode(tr("Stabilized")), _rattitudeFlightMode(tr("Rattitude")), _altCtlFlightMode(tr("Altitude")), _posCtlFlightMode(tr("Position")),
      _offboardFlightMode(tr("Offboard")), _readyFlightMode(tr("Ready")), _takeoffFlightMode(tr("Takeoff")), _holdFlightMode(tr("Hold")), _missionFlightMode(tr("Mission")), _rtlFlightMode(tr("Return")), _landingFlightMode(tr("Land")),
      _preclandFlightMode(tr("Precision Land")), _rtgsFlightMode(tr("Return to Groundstation")), _followMeFlightMode(tr("Follow Me")), _simpleFlightMode(tr("Simple")), _orbitFlightMode(tr("Orbit")) {
    ...
    const QString *rgModeNames[] = {
        &_manualFlightMode, &_stabilizedFlightMode, &_acroFlightMode, &_rattitudeFlightMode, &_altCtlFlightMode,  &_offboardFlightMode, &_simpleFlightMode, &_posCtlFlightMode, &_orbitFlightMode,
        &_holdFlightMode,   &_missionFlightMode,    &_rtlFlightMode,  &_followMeFlightMode,  &_landingFlightMode, &_preclandFlightMode, &_readyFlightMode,  &_rtgsFlightMode,   &_takeoffFlightMode,
    };
...
AutoPilotPlugin *PX4FirmwarePlugin::autopilotPlugin(Vehicle *vehicle) { return new PX4AutoPilotPlugin(vehicle, vehicle); }
...
void PX4FirmwarePlugin::initializeVehicle(Vehicle *vehicle) { vehicle->setFirmwarePluginInstanceData(new PX4FirmwarePluginInstanceData); }
...
void PX4FirmwarePlugin::_getParameterMetaDataVersionInfo(const QString &metaDataFile, int &majorVersion, int &minorVersion) { return PX4ParameterMetaData::getParameterMetaDataVersionInfo(metaDataFile, majorVersion, minorVersion); }

QList<MAV_CMD> PX4FirmwarePlugin::supportedMissionCommands(QGCMAVLink::VehicleClass_t vehicleClass) const {
    QList<MAV_CMD> supportedCommands = { MAV_CMD_NAV_WAYPOINT,
...
QString PX4FirmwarePlugin::missionCommandOverrides(QGCMAVLink::VehicleClass_t vehicleClass) const {
    switch (vehicleClass) {
        case QGCMAVLink::VehicleClassGeneric: return QStringLiteral(":/json/PX4-MavCmdInfoCommon.json");
        case QGCMAVLink::VehicleClassFixedWing: return QStringLiteral(":/json/PX4-MavCmdInfoFixedWing.json");
        case QGCMAVLink::VehicleClassMultiRotor: return QStringLiteral(":/json/PX4-MavCmdInfoMultiRotor.json");
        case QGCMAVLink::VehicleClassVTOL: return QStringLiteral(":/json/PX4-MavCmdInfoVTOL.json");
        case QGCMAVLink::VehicleClassSub: return QStringLiteral(":/json/PX4-MavCmdInfoSub.json");
        case QGCMAVLink::VehicleClassRoverBoat: return QStringLiteral(":/json/PX4-MavCmdInfoRover.json");
        default: qWarning() << "PX4FirmwarePlugin::missionCommandOverrides called with bad VehicleClass_t:" << vehicleClass; return QString();
    }
}
...
bool PX4FirmwarePlugin::mulirotorSpeedLimitsAvailable(Vehicle *vehicle) { return vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "MPC_XY_VEL_MAX"); }

bool PX4FirmwarePlugin::fixedWingAirSpeedLimitsAvailable(Vehicle *vehicle) {
    return vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "FW_AIRSPD_MIN") && vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "FW_AIRSPD_MAX");
}
...
void PX4FirmwarePlugin::guidedModeGotoLocation(Vehicle *vehicle, const QGeoCoordinate &gotoCoord) {
    ...
    if (vehicle->capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        vehicle->sendMavCommandInt(
            vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION, MAV_FRAME_GLOBAL,
            true, // show error is fails
            -1.0f, MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, 0.0f, NAN, gotoCoord.latitude(), gotoCoord.longitude(), vehicle->altitudeAMSL()->rawValue().toFloat()
        );
    } else {
        vehicle->sendMavCommand(
            vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION,
            true, // show error is fails
            -1.0f, MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, 0.0f, NAN, static_cast<float>(gotoCoord.latitude()), static_cast<float>(gotoCoord.longitude()), vehicle->altitudeAMSL()->rawValue().toFloat()
        );
    }
}
...
static void _pauseVehicleThenChangeAltResultHandler(void *resultHandlerData, int /*compId*/, const mavlink_command_ack_t &ack, Vehicle::MavCmdResultFailureCode_t failureCode) {
    if (ack.result != MAV_RESULT_ACCEPTED) {
        switch (failureCode) {
            case Vehicle::MavCmdResultCommandResultOnly: qDebug() << QStringLiteral("MAV_CMD_DO_REPOSITION error(%1)").arg(ack.result); break;
            case Vehicle::MavCmdResultFailureNoResponseToCommand: qDebug() << "MAV_CMD_DO_REPOSITION no response from vehicle"; break;
            case Vehicle::MavCmdResultFailureDuplicateCommand: qDebug() << "Internal Error: MAV_CMD_DO_REPOSITION could not be sent due to duplicate command"; break;
        }
    }
...
void PX4FirmwarePlugin::_changeAltAfterPause(void *resultHandlerData, bool pauseSucceeded) {
    ...
    if (pauseSucceeded) {
        pData->vehicle->sendMavCommand(
            pData->vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION,
            true,  // show error is fails
...
void PX4FirmwarePlugin::guidedModeChangeAltitude(Vehicle *vehicle, double altitudeChange, bool pauseVehicle) {
    ...
    if (pauseVehicle) {
        ...
        vehicle->sendMavCommandWithHandler(
            &handlerInfo, vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION,
            -1.0f, // Don't change groundspeed
            MAV_DO_REPOSITION_FLAGS_CHANGE_MODE,
            0.0f, // Reserved
            qQNaN(), qQNaN(), qQNaN(), qQNaN()
        ); // No change to yaw, lat, lon, alt
    } else {
...
void PX4FirmwarePlugin::guidedModeChangeHeading(Vehicle *vehicle, const QGeoCoordinate &headingCoord) {
    ...
    vehicle->sendMavCommand(
        vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION, true,
...
bool PX4FirmwarePlugin::isGuidedMode(const Vehicle *vehicle) const {
    ...
    return (vehicle->flightMode() == _holdFlightMode || vehicle->flightMode() == _takeoffFlightMode || vehicle->flightMode() == _landingFlightMode);
}
...
bool PX4FirmwarePlugin::adjustIncomingMavlinkMessage(Vehicle *vehicle, mavlink_message_t *message) {
    ...
    switch (message->msgid) {
        case MAVLINK_MSG_ID_AUTOPILOT_VERSION: _handleAutopilotVersion(vehicle, message); break;
    }
...
void PX4FirmwarePlugin::_handleAutopilotVersion(Vehicle *vehicle, mavlink_message_t *message) {
    ...
    if (!instanceData->versionNotified) {
        ...
        if (notifyUser) {
            ...
            qgcApp()->showAppMessage(tr("QGroundControl supports PX4 Pro firmware Version %1.%2.%3 and above. You are using a version prior to that which will lead to unpredictable results. Please upgrade your firmware.")
...
bool PX4FirmwarePlugin::supportsNegativeThrust(Vehicle *vehicle) { return ((vehicle->vehicleType() == MAV_TYPE_GROUND_ROVER) || (vehicle->vehicleType() == MAV_TYPE_SUBMARINE)); }
...
QString PX4FirmwarePlugin::getHobbsMeter(Vehicle *vehicle) {
    ...
    if (vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, HOOBS_HI) && vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, HOOBS_LO)) {
...
QVariant PX4FirmwarePlugin::mainStatusIndicatorContentItem(const Vehicle *) const { return QVariant::fromValue(QUrl::fromUserInput("qrc:/PX4/Indicators/PX4MainStatusIndicatorContentItem.qml")); }
