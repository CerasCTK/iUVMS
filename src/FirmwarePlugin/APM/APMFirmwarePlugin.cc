 ...
 */
APMCustomMode::APMCustomMode(uint32_t mode, bool settable) : _mode(mode), _settable(settable) {}

void APMCustomMode::setEnumToStringMapping(const QMap<uint32_t, QString> &enumToString) { _enumToString = enumToString; }
...
APMFirmwarePlugin::APMFirmwarePlugin(void) : _coaxialMotors(false) {
...
AutoPilotPlugin *APMFirmwarePlugin::autopilotPlugin(Vehicle *vehicle) { return new APMAutoPilotPlugin(vehicle, vehicle); }
...
void APMFirmwarePlugin::_handleIncomingParamValue(Vehicle *vehicle, mavlink_message_t *message) {
    ...
    switch (paramValue.param_type) {
        case MAV_PARAM_TYPE_UINT8: paramUnion.param_uint8 = static_cast<uint8_t>(paramValue.param_value); break;
        case MAV_PARAM_TYPE_INT8: paramUnion.param_int8 = static_cast<int8_t>(paramValue.param_value); break;
        case MAV_PARAM_TYPE_UINT16: paramUnion.param_uint16 = static_cast<uint16_t>(paramValue.param_value); break;
        case MAV_PARAM_TYPE_INT16: paramUnion.param_int16 = static_cast<int16_t>(paramValue.param_value); break;
        case MAV_PARAM_TYPE_UINT32: paramUnion.param_uint32 = static_cast<uint32_t>(paramValue.param_value); break;
        case MAV_PARAM_TYPE_INT32: paramUnion.param_int32 = static_cast<int32_t>(paramValue.param_value); break;
        case MAV_PARAM_TYPE_REAL32: paramUnion.param_float = paramValue.param_value; break;
        default: qCCritical(APMFirmwarePluginLog) << "Invalid/Unsupported data type used in parameter:" << paramValue.param_type;
    }
...
void APMFirmwarePlugin::_handleOutgoingParamSetThreadSafe(Vehicle * /*vehicle*/, LinkInterface *outgoingLink, mavlink_message_t *message) {
    ...
    switch (paramSet.param_type) {
        case MAV_PARAM_TYPE_UINT8: paramSet.param_value = paramUnion.param_uint8; break;
        case MAV_PARAM_TYPE_INT8: paramSet.param_value = paramUnion.param_int8; break;
        case MAV_PARAM_TYPE_UINT16: paramSet.param_value = paramUnion.param_uint16; break;
        case MAV_PARAM_TYPE_INT16: paramSet.param_value = paramUnion.param_int16; break;
        case MAV_PARAM_TYPE_UINT32: paramSet.param_value = paramUnion.param_uint32; break;
        case MAV_PARAM_TYPE_INT32: paramSet.param_value = paramUnion.param_int32; break;
        case MAV_PARAM_TYPE_REAL32:
            // Already in param_float
            break;
        default: qCCritical(APMFirmwarePluginLog) << "Invalid/Unsupported data type used in parameter:" << paramSet.param_type;
    }
    ...
    if (message->msgid == MAVLINK_MSG_ID_HEARTBEAT) {
        ...
        if (_ardupilotComponentMap[vehicle->id()][message->compid]) {
            switch (message->msgid) {
                case MAVLINK_MSG_ID_PARAM_VALUE: _handleIncomingParamValue(vehicle, message); break;
                case MAVLINK_MSG_ID_STATUSTEXT: return _handleIncomingStatusText(vehicle, message);
                case MAVLINK_MSG_ID_RC_CHANNELS: _handleRCChannels(vehicle, message); break;
                case MAVLINK_MSG_ID_RC_CHANNELS_RAW: _handleRCChannelsRaw(vehicle, message); break;
            }
        }
    }
...
void APMFirmwarePlugin::adjustOutgoingMavlinkMessageThreadSafe(Vehicle *vehicle, LinkInterface *outgoingLink, mavlink_message_t *message) {
    switch (message->msgid) {
        case MAVLINK_MSG_ID_PARAM_SET: _handleOutgoingParamSetThreadSafe(vehicle, outgoingLink, message); break;
    }
}
...
void APMFirmwarePlugin::initializeStreamRates(Vehicle *vehicle) {
    ...
    if (SettingsManager::instance()->appSettings()->apmStartMavlinkStreams()->rawValue().toBool()) {
        ...
        StreamInfo_s rgStreamInfo[] = {
            { MAV_DATA_STREAM_RAW_SENSORS, streamRates->streamRateRawSensors()->rawValue().toInt() }, { MAV_DATA_STREAM_EXTENDED_STATUS, streamRates->streamRateExtendedStatus()->rawValue().toInt() },
            { MAV_DATA_STREAM_RC_CHANNELS, streamRates->streamRateRCChannels()->rawValue().toInt() }, { MAV_DATA_STREAM_POSITION, streamRates->streamRatePosition()->rawValue().toInt() },
            { MAV_DATA_STREAM_EXTRA1, streamRates->streamRateExtra1()->rawValue().toInt() },          { MAV_DATA_STREAM_EXTRA2, streamRates->streamRateExtra2()->rawValue().toInt() },
...
void APMFirmwarePlugin::initializeVehicle(Vehicle *vehicle) {
    if (vehicle->isOfflineEditingVehicle()) {
        switch (vehicle->vehicleType()) {
            ...
            case MAV_TYPE_HELICOPTER: vehicle->setFirmwareVersion(3, 6, 0); break;
            ...
            case MAV_TYPE_FIXED_WING: vehicle->setFirmwareVersion(3, 9, 0); break;
            case MAV_TYPE_GROUND_ROVER:
            case MAV_TYPE_SURFACE_BOAT: vehicle->setFirmwareVersion(3, 5, 0); break;
            case MAV_TYPE_SUBMARINE: vehicle->setFirmwareVersion(3, 4, 0); break;
            default:
                // No version set
                break;
        }
...
void APMFirmwarePlugin::_getParameterMetaDataVersionInfo(const QString &metaDataFile, int &majorVersion, int &minorVersion) { APMParameterMetaData::getParameterMetaDataVersionInfo(metaDataFile, majorVersion, minorVersion); }
...
QString APMFirmwarePlugin::missionCommandOverrides(QGCMAVLink::VehicleClass_t vehicleClass) const {
    switch (vehicleClass) {
        ...
        default: qWarning() << "APMFirmwarePlugin::missionCommandOverrides called with bad VehicleClass_t:" << vehicleClass; return QString();
    }
}
...
void APMFirmwarePlugin::_artooSocketError(QAbstractSocket::SocketError socketError) { qgcApp()->showAppMessage(tr("Error during Solo video link setup: %1").arg(socketError)); }

QString APMFirmwarePlugin::_vehicleClassToString(QGCMAVLink::VehicleClass_t vehicleClass) const {
    switch (vehicleClass) {
        case QGCMAVLink::VehicleClassMultiRotor: return QStringLiteral("Copter");

        case QGCMAVLink::VehicleClassFixedWing:
        case QGCMAVLink::VehicleClassVTOL: return QStringLiteral("Plane");

        case QGCMAVLink::VehicleClassRoverBoat: return QStringLiteral("Rover");

        case QGCMAVLink::VehicleClassSub: return QStringLiteral("Sub");

        default: qCWarning(APMFirmwarePluginLog) << Q_FUNC_INFO << "called with bad VehicleClass_t:" << vehicleClass; return QString();
    }
}
...
void APMFirmwarePlugin::guidedModeGotoLocation(Vehicle *vehicle, const QGeoCoordinate &gotoCoord) {
    ...
    if (instanceData) {
        if (instanceData->MAV_CMD_DO_REPOSITION_supported || !instanceData->MAV_CMD_DO_REPOSITION_unsupported) {
            auto *result_handler_data = new MAV_CMD_DO_REPOSITION_HandlerData_t{ vehicle };
            ...
            vehicle->sendMavCommandIntWithHandler(
                &handlerInfo, vehicle->defaultComponentId(), MAV_CMD_DO_REPOSITION, MAV_FRAME_GLOBAL, -1.0f, MAV_DO_REPOSITION_FLAGS_CHANGE_MODE, 0.0f, NAN, gotoCoord.latitude(), gotoCoord.longitude(),
...
void APMFirmwarePlugin::guidedModeRTL(Vehicle *vehicle, bool smartRTL) { _setFlightModeAndValidate(vehicle, smartRTL ? smartRTLFlightMode() : rtlFlightMode()); }
...
void APMFirmwarePlugin::guidedModeChangeAltitude(Vehicle *vehicle, double altitudeChange, bool pauseVehicle) {
    ...
    if (sharedLink) {
        ...
        mavlink_msg_set_position_target_local_ned_encode_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()), static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &msg, &cmd);

        vehicle->sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
    }
}

bool APMFirmwarePlugin::mulirotorSpeedLimitsAvailable(Vehicle *vehicle) { return vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "WPNAV_SPEED"); }
...
void APMFirmwarePlugin::guidedModeChangeGroundSpeedMetersSecond(Vehicle *vehicle, double groundspeed) {
    vehicle->sendMavCommand(
        vehicle->defaultComponentId(), MAV_CMD_DO_CHANGE_SPEED,
...
void APMFirmwarePlugin::guidedModeTakeoff(Vehicle *vehicle, double altitudeRel) { _guidedModeTakeoff(vehicle, altitudeRel); }
...
void APMFirmwarePlugin::guidedModeChangeHeading(Vehicle *vehicle, const QGeoCoordinate &headingCoord) {
    ...
    vehicle->sendMavCommand(vehicle->defaultComponentId(), MAV_CMD_CONDITION_YAW, true, diff, maxYawRate, direction, true);
}
...
bool APMFirmwarePlugin::_guidedModeTakeoff(Vehicle *vehicle, double altitudeRel) {
    ...
    vehicle->sendMavCommand(
        vehicle->defaultComponentId(), MAV_CMD_NAV_TAKEOFF,
        true, // show error
...
void APMFirmwarePlugin::_handleRCChannels(Vehicle *vehicle, mavlink_message_t *message) {
    ...
    if (sharedLink) {
        ...
        mavlink_msg_rc_channels_encode_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()), static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), message, &channels);
    }
}
...
void APMFirmwarePlugin::_handleRCChannelsRaw(Vehicle *vehicle, mavlink_message_t *message) {
    ...
    if (sharedLink) {
        ...
        mavlink_msg_rc_channels_raw_encode_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()), static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), message, &channels);
    }
}
...
void APMFirmwarePlugin::sendGCSMotionReport(Vehicle *vehicle, FollowMe::GCSMotionReport &motionReport, uint8_t estimationCapabilities) {
    ...
    (void)mavlink_msg_global_position_int_encode_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()), static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &message, &globalPositionInt);
...
bool APMFirmwarePlugin::fixedWingAirSpeedLimitsAvailable(Vehicle *vehicle) {
    return vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "ARSPD_FBW_MIN") && vehicle->parameterManager()->parameterExists(ParameterManager::defaultComponentId, "ARSPD_FBW_MAX");
}
...
QVariant APMFirmwarePlugin::mainStatusIndicatorContentItem(const Vehicle *) const { return QVariant::fromValue(QUrl::fromUserInput("qrc:/APM/Indicators/APMMainStatusIndicatorContentItem.qml")); }
