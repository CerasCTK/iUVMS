...
GstGLDisplay *gst_qml6_get_gl_display(gboolean sink) {
    ...
    if (g_once_init_enter(&_debug)) {
        GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "qtglutility", 0, "Qt gl utility functions");
    ...
    if (QString::fromUtf8("xcb") == app->platformName()) {
        ...
        if (x11_native) {
            display = (GstGLDisplay *)gst_gl_display_x11_new_with_display(x11_native->display());
        }
    }
#endif
#if GST_GL_HAVE_WINDOW_WAYLAND && GST_GL_HAVE_PLATFORM_EGL && defined(HAVE_QT_WAYLAND)
    if (QString::fromUtf8("wayland") == app->platformName() || QString::fromUtf8("wayland-egl") == app->platformName()) {
        struct wl_display *wayland_display;
        QPlatformNativeInterface *native = QGuiApplication::platformNativeInterface();
        wayland_display = (struct wl_display *)native->nativeResourceForWindow("display", NULL);
        display = (GstGLDisplay *)gst_gl_display_wayland_new_with_display(wayland_display);
    }
#endif
    ...
    if (QString::fromUtf8("eglfs") == app->platformName()) {
        ...
        QPlatformNativeInterface *native = QGuiApplication::platformNativeInterface();
        EGLDisplay egl_display = (EGLDisplay)native->nativeResourceForWindow("egldisplay", NULL);
...
gboolean gst_qml6_get_gl_wrapcontext(GstGLDisplay *display, GstGLContext **wrap_glcontext, GstGLContext **context) {
    ...
    if (current && current->display == display) {
        ...
        *wrap_glcontext = static_cast<GstGLContext *>(gst_object_ref(current));
        ...
        if (gl_handle)
            *wrap_glcontext = gst_gl_context_new_wrapped(display, gl_handle, platform, gl_api);
