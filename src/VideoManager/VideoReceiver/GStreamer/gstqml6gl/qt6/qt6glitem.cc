...
void Qt6GLVideoItem::setDAR(gint num, gint den) {
    this->priv->par_n = num;
    this->priv->par_d = den;
}

void Qt6GLVideoItem::getDAR(gint *num, gint *den) {
    if (num)
        *num = this->priv->par_n;
    if (den)
        *den = this->priv->par_d;
...
void Qt6GLVideoItem::setForceAspectRatio(bool force_aspect_ratio) {
    this->priv->force_aspect_ratio = !!force_aspect_ratio;

    emit forceAspectRatioChanged(force_aspect_ratio);
}

bool Qt6GLVideoItem::getForceAspectRatio() { return this->priv->force_aspect_ratio; }

bool Qt6GLVideoItem::itemInitialized() { return this->priv->initted; }
...
static gboolean _calculate_par(Qt6GLVideoItem *widget, GstVideoInfo *info) {
    ...
    ok = gst_video_calculate_display_ratio(&display_ratio_num, &display_ratio_den, width, height, par_n, par_d, display_par_n, display_par_d);
    ...
    GST_LOG("%p PAR: %u/%u DAR:%u/%u", widget, par_n, par_d, display_par_n, display_par_d);

    if (height % display_ratio_den == 0) {
        GST_DEBUG("%p keeping video height", widget);
        widget->priv->display_width = (guint)gst_util_uint64_scale_int(height, display_ratio_num, display_ratio_den);
        widget->priv->display_height = height;
    } else if (width % display_ratio_num == 0) {
        GST_DEBUG("%p keeping video width", widget);
        widget->priv->display_width = width;
        widget->priv->display_height = (guint)gst_util_uint64_scale_int(width, display_ratio_den, display_ratio_num);
    } else {
        GST_DEBUG("%p approximating while keeping video height", widget);
        widget->priv->display_width = (guint)gst_util_uint64_scale_int(height, display_ratio_num, display_ratio_den);
        widget->priv->display_height = height;
    }
    GST_DEBUG("%p scaling to %dx%d", widget, widget->priv->display_width, widget->priv->display_height);

    return TRUE;
}

QSGNode *Qt6GLVideoItem::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *updatePaintNodeData) {
...
void Qt6GLVideoItem::fitStreamToAllocatedSize(GstVideoRectangle *result) {
...
/* This method has to be invoked with the the priv->lock taken */
QPointF Qt6GLVideoItem::mapPointToStreamSize(QPointF pos) {
...
void Qt6GLVideoItem::wheelEvent(QWheelEvent *event) {
    // noop
}

void Qt6GLVideoItem::hoverEnterEvent(QHoverEvent *) { mouseHovering = true; }

void Qt6GLVideoItem::hoverLeaveEvent(QHoverEvent *) { mouseHovering = false; }

void Qt6GLVideoItem::hoverMoveEvent(QHoverEvent *event) {
    // noop
}

void Qt6GLVideoItem::touchEvent(QTouchEvent *event) {
    // noop
}

void Qt6GLVideoItem::sendMouseEvent(QMouseEvent *event, gboolean is_press) {
    // noop
}

void Qt6GLVideoItem::mousePressEvent(QMouseEvent *event) {
    forceActiveFocus();
    sendMouseEvent(event, TRUE);
}

void Qt6GLVideoItem::mouseReleaseEvent(QMouseEvent *event) { sendMouseEvent(event, FALSE); }
...
void Qt6GLVideoItemInterface::setBuffer(GstBuffer *buffer) {
    ...
    if (qt_item->priv->new_caps) {
        GST_DEBUG("%p caps change from %" GST_PTR_FORMAT " to %" GST_PTR_FORMAT, this, qt_item->priv->caps, qt_item->priv->new_caps);
...
void Qt6GLVideoItem::onSceneGraphInitialized() {
    ...
    gl_context = static_cast<QOpenGLContext *>(renderer->getResource(this->window(), QSGRendererInterface::Resource::OpenGLContextResource));

    GST_DEBUG("%p scene graph initialization with Qt GL context %p", this, gl_context);
    ...
    this->priv->initted = gst_qml6_get_gl_wrapcontext(this->priv->display, &this->priv->other_context, &this->priv->context);

    GST_DEBUG("%p created wrapped GL context %" GST_PTR_FORMAT, this, this->priv->other_context);

    emit itemInitializedChanged();
}

void Qt6GLVideoItem::onSceneGraphInvalidated() {
...
gboolean Qt6GLVideoItemInterface::initWinSys() {
    ...
    if (qt_item->priv->display && qt_item->priv->qt_context && qt_item->priv->other_context && qt_item->priv->context) {
    ...
    if (!GST_IS_GL_DISPLAY(qt_item->priv->display)) {
        GST_ERROR("%p failed to retrieve display connection %" GST_PTR_FORMAT, qt_item, qt_item->priv->display);
        g_mutex_unlock(&qt_item->priv->lock);
        return FALSE;
    }

    if (!GST_IS_GL_CONTEXT(qt_item->priv->other_context)) {
        GST_ERROR("%p failed to retrieve wrapped context %" GST_PTR_FORMAT, qt_item, qt_item->priv->other_context);
        g_mutex_unlock(&qt_item->priv->lock);
        return FALSE;
    }
    ...
    if (!gst_gl_context_create(qt_item->priv->context, qt_item->priv->other_context, &error)) {
...
void Qt6GLVideoItem::handleWindowChanged(QQuickWindow *win) {
    if (win) {
        if (win->isSceneGraphInitialized())
            win->scheduleRenderJob(new RenderJob(std::bind(&Qt6GLVideoItem::onSceneGraphInitialized, this)), QQuickWindow::BeforeSynchronizingStage);
        else
...
void Qt6GLVideoItem::releaseResources() { this->priv->m_node = nullptr; }
...
void Qt6GLVideoItemInterface::setDAR(gint num, gint den) {
    QMutexLocker locker(&lock);
    if (!qt_item)
        return;
...
void Qt6GLVideoItemInterface::getDAR(gint *num, gint *den) {
    QMutexLocker locker(&lock);
    if (!qt_item)
        return;
