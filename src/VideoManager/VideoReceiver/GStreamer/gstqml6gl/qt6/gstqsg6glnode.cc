...
GstQSG6OpenGLNode::GstQSG6OpenGLNode(QQuickItem *item) {
    ...
    if (g_once_init_enter(&_debug)) {
        GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "qtqsgtexture", 0, "Qt Scenegraph Texture");
...
QSGTexture *GstQSG6OpenGLNode::texture() const { return QSGSimpleTextureNode::texture(); }
...
void GstQSG6OpenGLNode::setBuffer(GstBuffer *buffer) {
    ...
    if (buffer_changed) {
        ...
        if (!gst_video_frame_map(&this->v_frame, &this->v_info, this->buffer_, (GstMapFlags)(GST_MAP_READ | GST_MAP_GL))) {
