...
static GstStaticPadTemplate gst_qt_sink_template = GST_STATIC_PAD_TEMPLATE(
...
G_DEFINE_TYPE_WITH_CODE(GstQml6GLSink, gst_qml6_gl_sink, GST_TYPE_VIDEO_SINK, GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "qtsink", 0, "Qt Video Sink"); G_IMPLEMENT_INTERFACE(GST_TYPE_NAVIGATION, gst_qml6_gl_sink_navigation_interface_init));
...
static void gst_qml6_gl_sink_class_init(GstQml6GLSinkClass *klass) {
    ...
    gst_element_class_set_metadata(gstelement_class, "Qt6 Video Sink", "Sink/Video", "A video sink that renders to a QQuickItem for Qt6", "Matthew Waters <matthew@centricular.com>");

    g_object_class_install_property(gobject_class, PROP_WIDGET, g_param_spec_pointer("widget", "QQuickItem", "The QQuickItem to place in the object hierarchy", (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

    g_object_class_install_property(
        gobject_class, PROP_FORCE_ASPECT_RATIO,
        g_param_spec_boolean("force-aspect-ratio", "Force aspect ratio", "When enabled, scaling will respect original aspect ratio", DEFAULT_FORCE_ASPECT_RATIO, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS))
    );

    g_object_class_install_property(
        gobject_class, PROP_PIXEL_ASPECT_RATIO,
        gst_param_spec_fraction("pixel-aspect-ratio", "Pixel Aspect Ratio", "The pixel aspect ratio of the device", DEFAULT_PAR_N, DEFAULT_PAR_D, G_MAXINT, 1, 1, 1, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS))
    );
...
static void gst_qml6_gl_sink_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    ...
    switch (prop_id) {
        ...
        case PROP_PIXEL_ASPECT_RATIO:
            g_return_if_fail(qt_sink->widget);
            qt_sink->widget->setDAR(gst_value_get_fraction_numerator(value), gst_value_get_fraction_denominator(value));
            break;
...
static gboolean gst_qml6_gl_sink_query(GstBaseSink *bsink, GstQuery *query) {
    ...
    switch (GST_QUERY_TYPE(query)) {
        case GST_QUERY_CONTEXT: {
            if (gst_gl_handle_context_query((GstElement *)qt_sink, query, qt_sink->display, qt_sink->context, qt_sink->qt_context))
                return TRUE;

            /* fallthrough */
        }
        default: res = GST_BASE_SINK_CLASS(parent_class)->query(bsink, query); break;
    }
...
static GstStateChangeReturn gst_qml6_gl_sink_change_state(GstElement *element, GstStateChange transition) {
    ...
    GST_DEBUG("changing state: %s => %s", gst_element_state_get_name(GST_STATE_TRANSITION_CURRENT(transition)), gst_element_state_get_name(GST_STATE_TRANSITION_NEXT(transition)));

    switch (transition) {
        case GST_STATE_CHANGE_NULL_TO_READY:
            app = static_cast<QGuiApplication *>(QCoreApplication::instance());
            if (!app) {
                GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, ("%s", "Failed to connect to Qt"), ("%s", "Could not retrieve QGuiApplication instance"));
            ...
            if (!qt_sink->display || !qt_sink->context || !qt_sink->qt_context) {
                GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, ("%s", "Could not retrieve window system OpenGL configuration"), (NULL));
                return GST_STATE_CHANGE_FAILURE;
            }
    ...
    switch (transition) {
        ...
        case GST_STATE_CHANGE_PAUSED_TO_READY:
            ...
            break;
        case GST_STATE_CHANGE_READY_TO_NULL: break;
        default: break;
    }
        ...
        if (GST_BUFFER_DURATION_IS_VALID(buf))
            *end = *start + GST_BUFFER_DURATION(buf);
            ...
            if (GST_VIDEO_INFO_FPS_N(&qt_sink->v_info) > 0) {
                *end = *start + gst_util_uint64_scale_int(GST_SECOND, GST_VIDEO_INFO_FPS_D(&qt_sink->v_info), GST_VIDEO_INFO_FPS_N(&qt_sink->v_info));
            }
        }
    }
}
...
static void gst_qml6_gl_sink_navigation_interface_init(GstNavigationInterface *iface) { iface->send_event = gst_qml6_gl_sink_navigation_send_event; }
