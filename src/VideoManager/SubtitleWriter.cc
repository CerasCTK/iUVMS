...
SubtitleWriter::SubtitleWriter(QObject *parent) : QObject(parent), _timer(new QTimer(this)) {
...
void SubtitleWriter::_captureTelemetry() {
    ...
    for (int i = 0; i < nRows; i++) {
        ...
        QString names = namesLine.arg(-offsetFactor / 2 + rowWidth * (i + 1) - 10).arg(start.toString("H:mm:ss.zzz").chopped(2)).arg(end.toString("H:mm:ss.zzz").chopped(2)).arg(currentColumnNameStrings.join("\\N"));
        stringColumns << names;

        // Fill templates for values of column i
        QString values = valuesLine.arg(-offsetFactor / 2 + rowWidth * (i + 1)).arg(start.toString("H:mm:ss.zzz").chopped(2)).arg(end.toString("H:mm:ss.zzz").chopped(2)).arg(currentColumnValueStrings.join("\\N"));
