...
Gimbal::Gimbal(GimbalController *parent) : FactGroup(100, parent, ":/json/Vehicle/GimbalFact.json") { _initFacts(); }

Gimbal::Gimbal(const Gimbal &other) : FactGroup(100, other.parent(), ":/json/Vehicle/GimbalFact.json") {
    _initFacts();
    *this = other;
}
...
GimbalController::GimbalController(MAVLinkProtocol *mavlink, Vehicle *vehicle) : QObject(vehicle), _mavlink(mavlink), _vehicle(vehicle), _activeGimbal(nullptr) {
...
GimbalController::~GimbalController() { _gimbals.clearAndDeleteContents(); }
...
void GimbalController::_mavlinkMessageReceived(const mavlink_message_t &message) {
    ...
    switch (message.msgid) {
        ...
        case MAVLINK_MSG_ID_GIMBAL_MANAGER_INFORMATION: _handleGimbalManagerInformation(message); break;
        case MAVLINK_MSG_ID_GIMBAL_MANAGER_STATUS: _handleGimbalManagerStatus(message); break;
        case MAVLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS: _handleGimbalDeviceAttitudeStatus(message); break;
    }
}
...
void GimbalController::_handleGimbalManagerInformation(const mavlink_message_t &message) {
    ...
    if (information.gimbal_device_id == 0) {
        qCWarning(GimbalLog) << "_handleGimbalManagerInformation for invalid gimbal device: " << information.gimbal_device_id << ", from component id: " << message.compid;
        return;
    }
    ...
    if (!gimbal->_receivedInformation) {
        qCDebug(GimbalLog) << "gimbal manager with compId: " << message.compid << " is responsible for gimbal device: " << information.gimbal_device_id;
    }
...
void GimbalController::_handleGimbalManagerStatus(const mavlink_message_t &message) {
    ...
    if (status.gimbal_device_id == 0) {
        qCDebug(GimbalLog) << "gimbal manager with compId: " << message.compid << " reported status of gimbal device id: " << status.gimbal_device_id << " which is not a valid gimbal device id";
        return;
    }
    ...
    if (!gimbal->_receivedStatus) {
        qCDebug(GimbalLog) << "_handleGimbalManagerStatus: gimbal manager with compId " << message.compid << " is responsible for gimbal device " << status.gimbal_device_id;
    }

    gimbal->_receivedStatus = true;

    const bool haveControl = (status.primary_control_sysid == _mavlink->getSystemId()) && (status.primary_control_compid == _mavlink->getComponentId());

    const bool othersHaveControl = !haveControl && (status.primary_control_sysid != 0 && status.primary_control_compid != 0);
...
void GimbalController::_handleGimbalDeviceAttitudeStatus(const mavlink_message_t &message) {
    ...
    if (attitude_status.gimbal_device_id == 0) {
        ...
        auto foundGimbal = std::find_if(_potentialGimbals.begin(), _potentialGimbals.end(), [&](auto &gimbal) { return gimbal.deviceId()->rawValue().toUInt() == pairId.deviceId; });

        if (foundGimbal == _potentialGimbals.end()) {
            qCDebug(GimbalLog) << "_handleGimbalDeviceAttitudeStatus for unknown device id: " << pairId.deviceId << " from component id: " << message.compid;
            return;
        }
...
void GimbalController::_requestGimbalInformation(uint8_t compid) {
    ...
    if (_vehicle) {
        _vehicle->sendMavCommand(compid, MAV_CMD_REQUEST_MESSAGE, false /* no error */, MAVLINK_MSG_ID_GIMBAL_MANAGER_INFORMATION);
    }
}
...
void GimbalController::_checkComplete(Gimbal &gimbal, GimbalPairId pairId) {
    ...
    if (!gimbal._receivedStatus && gimbal._requestStatusRetries > 0 && now - lastRequestStatusMessage > 1000) {
        ...
        _vehicle->sendMavCommand(
            pairId.managerCompid, MAV_CMD_SET_MESSAGE_INTERVAL, false /* no error */, MAVLINK_MSG_ID_GIMBAL_MANAGER_STATUS,
        ...
        qCDebug(GimbalLog) << "attempt to set GIMBAL_MANAGER_STATUS message at" << (gimbal._requestStatusRetries > 2 ? "default rate" : "0.2 Hz") << " interval for device: " << gimbal.deviceId()->rawValue().toUInt()
                           << "manager compID: " << pairId.managerCompid << ", retries remaining: " << gimbal._requestStatusRetries;
    }

    if (!gimbal._receivedAttitude && gimbal._requestAttitudeRetries > 0 && gimbal._receivedInformation && pairId.deviceId != 0) {
        ...
        _vehicle->sendMavCommand(gimbalDeviceCompid, MAV_CMD_SET_MESSAGE_INTERVAL, false /* no error */, MAVLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS, 0 /* request default rate */);
...
void GimbalController::sendPitchBodyYaw(float pitch, float yaw, bool showError) {
    ...
    unsigned flags = GIMBAL_MANAGER_FLAGS_ROLL_LOCK | GIMBAL_MANAGER_FLAGS_PITCH_LOCK | GIMBAL_MANAGER_FLAGS_YAW_IN_VEHICLE_FRAME;

    _vehicle->sendMavCommand(_activeGimbal->managerCompid()->rawValue().toUInt(), MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW, showError, pitch, yaw, NAN, NAN, flags, 0, _activeGimbal->deviceId()->rawValue().toUInt());
}
...
void GimbalController::sendPitchAbsoluteYaw(float pitch, float yaw, bool showError) {
    ...
    unsigned flags = GIMBAL_MANAGER_FLAGS_ROLL_LOCK | GIMBAL_MANAGER_FLAGS_PITCH_LOCK | GIMBAL_MANAGER_FLAGS_YAW_LOCK | GIMBAL_MANAGER_FLAGS_YAW_IN_EARTH_FRAME;
...
void GimbalController::sendRate() {
    ...
    _vehicle->sendMavCommand(
        _activeGimbal->managerCompid()->rawValue().toUInt(), MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW, false, NAN, NAN, _activeGimbal->pitchRate(), _activeGimbal->yawRate(), flags, 0, _activeGimbal->deviceId()->rawValue().toUInt()
    );
...
void GimbalController::sendPitchYawFlags(uint32_t flags) {
    ...
    _vehicle->sendMavCommand(
        _activeGimbal->managerCompid()->rawValue().toUInt(), MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW, true, _activeGimbal->absolutePitch()->rawValue().toFloat(),
        yaw_in_vehicle_frame ? _activeGimbal->bodyYaw()->rawValue().toFloat() : _activeGimbal->absoluteYaw()->rawValue().toFloat(), NAN, NAN, flags, 0, _activeGimbal->deviceId()->rawValue().toUInt()
    );
}
...
void GimbalController::acquireGimbalControl() {
    ...
    _vehicle->sendMavCommand(
        _activeGimbal->managerCompid()->rawValue().toUInt(), MAV_CMD_DO_GIMBAL_MANAGER_CONFIGURE, true,
