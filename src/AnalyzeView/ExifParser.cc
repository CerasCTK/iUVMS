...
namespace ExifParser {
    ...
    bool write(QByteArray &buf, const GeoTagWorker::CameraFeedbackPacket &geotag) {
        try {
            ...
            exifData["Exif.GPSInfo.GPSLatitude"] = std::to_string(latDegrees) + "/1 " + std::to_string(latMinutes) + "/1 " + std::to_string(static_cast<int>(latSeconds * 1000)) + "/1000";
            ...
            exifData["Exif.GPSInfo.GPSLongitude"] = std::to_string(lonDegrees) + "/1 " + std::to_string(lonMinutes) + "/1 " + std::to_string(static_cast<int>(lonSeconds * 1000)) + "/1000";
