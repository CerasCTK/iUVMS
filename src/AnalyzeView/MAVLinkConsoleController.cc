...
MAVLinkConsoleController::MAVLinkConsoleController(QObject *parent) : QStringListModel(parent), _palette(new QGCPalette(this)) {
...
void MAVLinkConsoleController::_sendSerialData(const QByteArray &data, bool close) {
    ...
    while (output.size()) {
        ...
        (void)mavlink_msg_serial_control_pack_chan(
            MAVLinkProtocol::instance()->getSystemId(), MAVLinkProtocol::getComponentId(), sharedLink->mavlinkChannel(), &msg, SERIAL_CONTROL_DEV_SHELL, flags, 0, 0, dataSize, reinterpret_cast<uint8_t *>(chunk.data()), _vehicle->id(),
