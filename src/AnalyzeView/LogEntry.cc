...
LogDownloadData::LogDownloadData(QGCLogEntry *const entry) : ID(entry->id()), entry(entry) {
...
bool LogDownloadData::chunkEquals(const bool val) const { return (chunk_table == QBitArray(chunk_table.size(), val)); }

/*===========================================================================*/

QGCLogEntry::QGCLogEntry(uint logId, const QDateTime &dateTime, uint logSize, bool received, QObject *parent) : QObject(parent), _logID(logId), _logSize(logSize), _logTimeUTC(dateTime), _received(received) {
    // qCDebug(LogEntryLog) << Q_FUNC_INFO << this;
}
...
QString QGCLogEntry::sizeStr() const { return qgcApp()->bigSizeToString(_logSize); }
