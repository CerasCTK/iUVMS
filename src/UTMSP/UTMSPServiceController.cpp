...
UTMSPServiceController::UTMSPServiceController(QObject *parent)
    : QObject(parent), _utmspNetworkRemoteIDManager(std::make_shared<Dispatcher>()), _activationFlag(false), _clientID(""), _clientPassword(""), _blenderToken(""), _lastLatitude(0), _lastLongitude(0), _lastAltitude(0) {}
...
bool UTMSPServiceController::networkRemoteID(const mavlink_message_t &message, const std::string &serialNumber, const std::string &operatorID, const std::string &flightID) {
    ...
    switch (message.msgid) {
        ...
        case MAVLINK_MSG_ID_GLOBAL_POSITION_INT: {
            ...
            _utmspNetworkRemoteIDManager.startTelemetry(
                _vehicleLatitude, _vehicleLongitude, _vehicleAltitude, _vehicleHeading, _vehicleVelocityX, _vehicleVelocityY, _vehicleVelocityZ, _vehicleRelativeAltitude, serialNumber, operatorID, flightID
            );
            _utmspFlightPlanManager.updateFlightPlanState(_currentState);
