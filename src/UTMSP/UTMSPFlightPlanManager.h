...
class UTMSPFlightPlanManager : public UTMSPBlenderRestInterface {
  public:
    ...
    enum class FlightState { Idle, Register, Activated, StartTelemetryStreaming, StopTelemetryStreaming };

    void getCapability();
    void preRegisterFlightPlan();
    void preRegisterFlightPlanNotification();
    void registerFlightPlan(const std::string &token, const json &boundaryPolygons, const int &minAltitude, const int &maxAltitude, const std::string &startDateTime, const std::string &endDateTime);
