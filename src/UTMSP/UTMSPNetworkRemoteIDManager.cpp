...
UTMSPNetworkRemoteIDManager::UTMSPNetworkRemoteIDManager(std::shared_ptr<Dispatcher> dispatcher) : _dispatcher(dispatcher) {}

UTMSPNetworkRemoteIDManager::~UTMSPNetworkRemoteIDManager() {}

void UTMSPNetworkRemoteIDManager::getCapabilty(const std::string &token) { setBearerToken(token.c_str()); }

void UTMSPNetworkRemoteIDManager::startTelemetry(
    const double &latitude, const double &longitude, const double &altitude, const double &heading, const double &velocityX, const double &velocityY, const double &velocityZ, const double &relativeAltitude,
    const std::string &aircraftSerialNumber, const std::string &operatorID, const std::string &flightID
) {
    // Generate RID JSON
    ...
    _flightDetailsJson["auth_data"] = { { "format", this->_flightDetails.auth_data_format }, { "data", this->_flightDetails.auth_data_data } };
    ...
    if (_statusCode == 201) {
        ...
        } catch (const json::parse_error &e) { UTMSP_LOG_ERROR() << "UTMSPNetworkRemoteManager: Error parsing the Telemetry response: " << e.what(); }
    } else {
