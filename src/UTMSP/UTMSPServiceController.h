...
class UTMSPServiceController : public QObject {
  ...
  public:
    ...
    bool networkRemoteID(const mavlink_message_t &message, const std::string &serialNumber, const std::string &operatorID, const std::string &flightID);
  ...
  public slots:
    ...
    void updatePolygonBoundary(const QList<QGeoCoordinate> &boundary) {
        _boundaryPolygon.clear();
        _boundaryPolygon.append(boundary);
    }
    ...
    void updateLastCoordinates(const double &lastLatitude, const double &lastLongitude) {
