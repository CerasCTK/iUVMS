...
//-----------------------------------------------------------------------------
QGCCameraOptionExclusion::QGCCameraOptionExclusion(QObject *parent, QString param_, QString value_, QStringList exclusions_) : QObject(parent), param(param_), value(value_), exclusions(exclusions_) {}
...
QGCCameraOptionRange::QGCCameraOptionRange(QObject *parent, QString param_, QString value_, QString targetParam_, QString condition_, QStringList optNames_, QStringList optValues_)
    : QObject(parent), param(param_), value(value_), targetParam(targetParam_), condition(condition_), optNames(optNames_), optValues(optValues_) {}
...
//-----------------------------------------------------------------------------
VehicleCameraControl::VehicleCameraControl(const mavlink_camera_information_t *info, Vehicle *vehicle, int compID, QObject *parent) : MavlinkCameraControl(parent), _vehicle(vehicle), _compID(compID) {
    ...
    _cacheFile = QString::asprintf("%s/%s_%s_%03d.xml", SettingsManager::instance()->appSettings()->parameterSavePath().toStdString().c_str(), _vendor.toStdString().c_str(), _modelName.toStdString().c_str(), ver);
...
//-----------------------------------------------------------------------------
QString VehicleCameraControl::firmwareVersion() {
...
//-----------------------------------------------------------------------------
QString VehicleCameraControl::recordTimeStr() { return QTime(0, 0).addMSecs(static_cast<int>(recordTime())).toString("hh:mm:ss"); }

//-----------------------------------------------------------------------------
VehicleCameraControl::VideoCaptureStatus VehicleCameraControl::videoCaptureStatus() { return _video_status; }

//-----------------------------------------------------------------------------
VehicleCameraControl::PhotoCaptureStatus VehicleCameraControl::photoCaptureStatus() { return _photo_status; }

//-----------------------------------------------------------------------------
QString VehicleCameraControl::storageFreeStr() { return qgcApp()->bigSizeMBToString(static_cast<quint64>(_storageFree)); }

//-----------------------------------------------------------------------------
QString VehicleCameraControl::batteryRemainingStr() {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::setCameraMode(CameraMode mode) {
    if (!_resetting) {
    ...
    if (!_resetting) {
        ...
        if (videoCaptureStatus() != VIDEO_CAPTURE_STATUS_RUNNING) {
            _vehicle->sendMavCommand(
                ...
                0,                           // All streams
                0
            ); // CAMERA_CAPTURE_STATUS streaming frequency
            return true;
        }
    }
    return false;
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::_mavCommandResult(int vehicleId, int component, int command, int result, bool noReponseFromVehicle) {
    //-- Is this ours?
    ...
    if (!noReponseFromVehicle && result == MAV_RESULT_IN_PROGRESS) {
        ...
        switch (command) {
            ...
            case MAV_CMD_VIDEO_STOP_CAPTURE:
                ...
                _captureStatusTimer.start(1000);
                break;
            case MAV_CMD_REQUEST_CAMERA_CAPTURE_STATUS: _captureInfoRetries = 0; break;
            case MAV_CMD_REQUEST_STORAGE_INFORMATION: _storageInfoRetries = 0; break;
            case MAV_CMD_IMAGE_START_CAPTURE: _captureStatusTimer.start(1000); break;
        }
    } else {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::_setVideoStatus(VideoCaptureStatus status) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::_setPhotoStatus(PhotoCaptureStatus status) {
    ...
    for (int i = 0; i < parameters.size(); i++) {
        ...
        if (read_attribute(parameterNode, kDefault, defaultValue)) {
            ...
            if (metaData->convertAndValidateRaw(defaultValue, false, defaultVariant, errorString)) {
                ...
                qWarning() << "Invalid default value for" << factName << " type:" << metaData->type() << " value:" << defaultValue << " error:" << errorString;
            }
        }
        ...
        } else {
            {
                ...
                if (read_attribute(parameterNode, kMin, attr)) {
                    ...
                    if (metaData->convertAndValidateRaw(attr, true /* convertOnly */, typedValue, errorString)) {
                        ...
                        qWarning() << "Invalid min value for" << factName << " type:" << metaData->type() << " value:" << attr << " error:" << errorString;
                    }
                }
            }
            {
                ...
                if (read_attribute(parameterNode, kMax, attr)) {
                    ...
                    if (metaData->convertAndValidateRaw(attr, true /* convertOnly */, typedValue, errorString)) {
                        ...
                        qWarning() << "Invalid max value for" << factName << " type:" << metaData->type() << " value:" << attr << " error:" << errorString;
                    }
                }
            }
            {
                ...
                if (read_attribute(parameterNode, kStep, attr)) {
                    ...
                    if (metaData->convertAndValidateRaw(attr, true /* convertOnly */, typedValue, errorString)) {
                        ...
                        qWarning() << "Invalid step value for" << factName << " type:" << metaData->type() << " value:" << attr << " error:" << errorString;
                    }
                }
            }
            {
                ...
                if (read_attribute(parameterNode, kDecimalPlaces, attr)) {
                    ...
                    if (metaData->convertAndValidateRaw(attr, true /* convertOnly */, typedValue, errorString)) {
                        ...
                        qWarning() << "Invalid decimal places value for" << factName << " type:" << metaData->type() << " value:" << attr << " error:" << errorString;
                    }
                }
            }
            {
...
void VehicleCameraControl::_requestAllParameters() {
    ...
    if (sharedLink) {
        ...
        mavlink_msg_param_ext_request_list_pack_chan(
            static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()), static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &msg, static_cast<uint8_t>(_vehicle->id()), static_cast<uint8_t>(compID())
        );
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleParamAck(const mavlink_param_ext_ack_t &ack) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleParamValue(const mavlink_param_ext_value_t &value) {
...
bool VehicleCameraControl::_processConditionTest(const QString conditionTest) {
    ...
    if (test.size() == 2) {
        ...
        if (pFact) {
            switch (op) {
                case TEST_EQUAL: return pFact->rawValueString() == test[1];
                case TEST_NOT_EQUAL: return pFact->rawValueString() != test[1];
                case TEST_GREATER: return pFact->rawValueString() > test[1];
                case TEST_SMALLER: return pFact->rawValueString() < test[1];
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleSettings(const mavlink_camera_settings_t &settings) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleStorageInfo(const mavlink_storage_information_t &st) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleCaptureStatus(const mavlink_camera_capture_status_t &cap) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleVideoInfo(const mavlink_video_stream_information_t *vi) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleVideoStatus(const mavlink_video_stream_status_t *vs) {
...
//-----------------------------------------------------------------------------
void VehicleCameraControl::handleTrackingImageStatus(const mavlink_camera_tracking_image_status_t *tis) {
    if (tis) {
        ...
        if (_trackingImageStatus.tracking_status == 0 || !trackingEnabled()) {
            ...
            if (_trackingImageStatus.tracking_mode == 2) {
                _trackingImageRect = QRectF(QPointF(_trackingImageStatus.rec_top_x, _trackingImageStatus.rec_top_y), QPointF(_trackingImageStatus.rec_bottom_x, _trackingImageStatus.rec_bottom_y));
            } else {
                ...
                _trackingImageRect = QRectF(QPointF(_trackingImageStatus.point_x - r, _trackingImageStatus.point_y - r), QPointF(_trackingImageStatus.point_x + r, NAN));
            }
            ...
            qCDebug(CameraControlLog) << "Tracking Image Status [left:" << _trackingImageRect.left() << "top:" << _trackingImageRect.top() << "right:" << _trackingImageRect.right() << "bottom:" << _trackingImageRect.bottom() << "]";
        }
...
//-----------------------------------------------------------------------------
bool VehicleCameraControl::autoStream() {
    if (hasVideoStream()) {
...
//-----------------------------------------------------------------------------
QGCVideoStreamInfo *VehicleCameraControl::_findStream(uint8_t id, bool report) {
    for (int i = 0; i < _streams.count(); i++) {
        if (_streams[i]) {
            QGCVideoStreamInfo *pStream = qobject_cast<QGCVideoStreamInfo *>(_streams[i]);
            if (pStream) {
...
//-----------------------------------------------------------------------------
QStringList VehicleCameraControl::_loadExclusions(QDomNode option) {
    QStringList exclusionList;
...
//-----------------------------------------------------------------------------
QStringList VehicleCameraControl::_loadUpdates(QDomNode option) {
        ...
        if (pRFact) {
            for (int i = 0; i < pRange->optNames.size(); i++) {
                ...
                if (!pRFact->metaData()->convertAndValidateRaw(pRange->optValues[i], false, optVariant, errorString)) {
                    qWarning() << "Invalid roption value, name:" << pRange->targetParam << " type:" << pRFact->metaData()->type() << " value:" << pRange->optValues[i] << " error:" << errorString;
                } else {
...
bool VehicleCameraControl::_loadNameValue(QDomNode option, const QString factName, FactMetaData *metaData, QString &optName, QString &optValue, QVariant &optVariant) {
    ...
    if (!metaData->convertAndValidateRaw(optValue, false, optVariant, errorString)) {
        qWarning() << "Invalid option value, name:" << factName << " type:" << metaData->type() << " value:" << optValue << " error:" << errorString;
    }
    return true;
}
...
void VehicleCameraControl::_handleDefinitionFile(const QString &url) {
    ...
    if (!xmlFile.exists() && url.startsWith(ftpPrefix, Qt::CaseInsensitive)) {
        ...
        QString fileName = QString::asprintf("%s_%s_%03d.xml%s", _vendor.toStdString().c_str(), _modelName.toStdString().c_str(), ver, ext.toStdString().c_str());
        connect(_vehicle->ftpManager(), &FTPManager::downloadComplete, this, &VehicleCameraControl::_ftpDownloadComplete);
        _vehicle->ftpManager()->download(_compID, url, SettingsManager::instance()->appSettings()->parameterSavePath().toStdString().c_str(), fileName);
        return;
    }
...
void VehicleCameraControl::_downloadFinished() {
    ...
    if (err == QNetworkReply::NoError && http_code == 200) {
        ...
        data.clear();
        qWarning() << QString("Camera Definition (%1) download error: %2 status: %3").arg(reply->url().toDisplayString(), reply->errorString(), reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toString());
    }
    emit dataReady(data);
    // reply->deleteLater();
}
...
//-----------------------------------------------------------------------------
QStringList VehicleCameraControl::activeSettings() {
    qCDebug(CameraControlLog) << "Active:" << _activeSettings;
    return _activeSettings;
}

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::exposureMode() { return (_paramComplete && _activeSettings.contains(kCAM_EXPMODE)) ? getFact(kCAM_EXPMODE) : nullptr; }

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::ev() { return (_paramComplete && _activeSettings.contains(kCAM_EV)) ? getFact(kCAM_EV) : nullptr; }

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::iso() { return (_paramComplete && _activeSettings.contains(kCAM_ISO)) ? getFact(kCAM_ISO) : nullptr; }

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::shutterSpeed() { return (_paramComplete && _activeSettings.contains(kCAM_SHUTTERSPD)) ? getFact(kCAM_SHUTTERSPD) : nullptr; }

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::aperture() { return (_paramComplete && _activeSettings.contains(kCAM_APERTURE)) ? getFact(kCAM_APERTURE) : nullptr; }

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::wb() { return (_paramComplete && _activeSettings.contains(kCAM_WBMODE)) ? getFact(kCAM_WBMODE) : nullptr; }

//-----------------------------------------------------------------------------
Fact *VehicleCameraControl::mode() { return _paramComplete && factExists(kCAM_MODE) ? getFact(kCAM_MODE) : nullptr; }

//-----------------------------------------------------------------------------
QGCVideoStreamInfo::QGCVideoStreamInfo(QObject *parent, const mavlink_video_stream_information_t *si) : QObject(parent) { memcpy(&_streamInfo, si, sizeof(mavlink_video_stream_information_t)); }
...
void VehicleCameraControl::startTracking(QRectF rec) {
    if (_trackingMarquee != rec) {
        ...
        qCDebug(CameraControlLog) << "Start Tracking (Rectangle: [" << static_cast<float>(rec.x()) << ", " << static_cast<float>(rec.y()) << "] - [" << static_cast<float>(rec.x() + rec.width()) << ", "
                                  << static_cast<float>(rec.y() + rec.height()) << "]";

        _vehicle->sendMavCommand(_compID, MAV_CMD_CAMERA_TRACK_RECTANGLE, true, static_cast<float>(rec.x()), static_cast<float>(rec.y()), static_cast<float>(rec.x() + rec.width()), static_cast<float>(rec.y() + rec.height()));
    }
}
...
void VehicleCameraControl::startTracking(QPointF point, double radius) {
    if (_trackingPoint != point || _trackingRadius != radius) {
        ...
        qCDebug(CameraControlLog) << "Start Tracking (Point: [" << static_cast<float>(point.x()) << ", " << static_cast<float>(point.y()) << "], Radius:  " << static_cast<float>(radius);

        _vehicle->sendMavCommand(_compID, MAV_CMD_CAMERA_TRACK_POINT, true, static_cast<float>(point.x()), static_cast<float>(point.y()), static_cast<float>(radius));
    }
}
...
void VehicleCameraControl::stopTracking() {
    ...
    _vehicle->sendMavCommand(_compID, MAV_CMD_SET_MESSAGE_INTERVAL, true, MAVLINK_MSG_ID_CAMERA_TRACKING_IMAGE_STATUS, -1);
