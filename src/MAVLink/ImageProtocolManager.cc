...
ImageProtocolManager::ImageProtocolManager(QObject *parent) : QObject(parent) {
...
bool ImageProtocolManager::requestImage(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t &message) {
    ...
    constexpr mavlink_data_transmission_handshake_t data = { 0, 0, 0, 0, MAVLINK_DATA_STREAM_IMG_JPEG, 0, 50 };
...
QImage ImageProtocolManager::_getImage() {
    ...
    switch (_imageHandshake.type) {
        ...
        default: qCWarning(ImageProtocolManagerLog) << Q_FUNC_INFO << "Unsupported image type:" << _imageHandshake.type; break;
    }
