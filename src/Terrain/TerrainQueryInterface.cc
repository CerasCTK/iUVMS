...
TerrainQueryInterface::TerrainQueryInterface(QObject *parent) : QObject(parent) {
...
void TerrainQueryInterface::signalCoordinateHeights(bool success, const QList<double> &heights) { emit coordinateHeightsReceived(success, heights); }

void TerrainQueryInterface::signalPathHeights(bool success, double distanceBetween, double finalDistanceBetween, const QList<double> &heights) { emit pathHeightsReceived(success, distanceBetween, finalDistanceBetween, heights); }

void TerrainQueryInterface::signalCarpetHeights(bool success, double minHeight, double maxHeight, const QList<QList<double>> &carpet) { emit carpetHeightsReceived(success, minHeight, maxHeight, carpet); }

void TerrainQueryInterface::_requestFailed() {
    switch (_queryMode) {
        case TerrainQuery::QueryModeCoordinates: emit coordinateHeightsReceived(false, QList<double>()); break;
        case TerrainQuery::QueryModePath: emit pathHeightsReceived(false, qQNaN(), qQNaN(), QList<double>()); break;
        case TerrainQuery::QueryModeCarpet: emit carpetHeightsReceived(false, qQNaN(), qQNaN(), QList<QList<double>>()); break;
        default: qCWarning(TerrainQueryInterfaceLog) << Q_FUNC_INFO << "Query Mode Not Supported"; break;
    }
}

/*===========================================================================*/

TerrainOfflineQuery::TerrainOfflineQuery(QObject *parent) : TerrainQueryInterface(parent) {
    // qCDebug(TerrainQueryInterfaceLog) << Q_FUNC_INFO << this;
}
...
TerrainOnlineQuery::TerrainOnlineQuery(QObject *parent) : TerrainQueryInterface(parent), _networkManager(new QNetworkAccessManager(this)) {
    // qCDebug(TerrainQueryInterfaceLog) << Q_FUNC_INFO << this;
