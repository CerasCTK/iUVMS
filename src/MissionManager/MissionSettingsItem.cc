...
MissionSettingsItem::MissionSettingsItem(PlanMasterController *masterController, bool flyView)
    : ComplexMissionItem(masterController, flyView), _managerVehicle(masterController->managerVehicle()), _plannedHomePositionAltitudeFact(0, _plannedHomePositionAltitudeName, FactMetaData::valueTypeDouble),
      _cameraSection(masterController), _speedSection(masterController) {
    _isIncomplete = false;
...
bool MissionSettingsItem::load(const QJsonObject & /*complexObject*/, int /*sequenceNumber*/, QString & /*errorString*/) { return true; }
...
void MissionSettingsItem::appendMissionItems(QList<MissionItem *> &items, QObject *missionItemParent) {
    ...
    MissionItem *item = new MissionItem(
        seqNum++, MAV_CMD_NAV_WAYPOINT, MAV_FRAME_GLOBAL,
        0, // Hold time
        0, // Acceptance radius
        0, // Not sure?
        0, // Yaw
...
bool MissionSettingsItem::addMissionEndAction(QList<MissionItem *> & /*items*/, int /*seqNum*/, QObject * /*missionItemParent*/) { return false; }
...
double MissionSettingsItem::specifiedGimbalYaw(void) { return _cameraSection.specifyGimbal() ? _cameraSection.gimbalYaw()->rawValue().toDouble() : std::numeric_limits<double>::quiet_NaN(); }

double MissionSettingsItem::specifiedGimbalPitch(void) { return _cameraSection.specifyGimbal() ? _cameraSection.gimbalPitch()->rawValue().toDouble() : std::numeric_limits<double>::quiet_NaN(); }
