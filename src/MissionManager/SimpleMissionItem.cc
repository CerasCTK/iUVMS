...
SimpleMissionItem::SimpleMissionItem(PlanMasterController *masterController, bool flyView, bool forLoad)
    : VisualMissionItem(masterController, flyView), _supportedCommandFact(0, "Command:", FactMetaData::valueTypeUint32), _altitudeFact(0, "Altitude", FactMetaData::valueTypeDouble),
      _amslAltAboveTerrainFact(0, "Alt above terrain", FactMetaData::valueTypeDouble), _param1MetaData(FactMetaData::valueTypeDouble), _param2MetaData(FactMetaData::valueTypeDouble), _param3MetaData(FactMetaData::valueTypeDouble),
      _param4MetaData(FactMetaData::valueTypeDouble), _param5MetaData(FactMetaData::valueTypeDouble), _param6MetaData(FactMetaData::valueTypeDouble), _param7MetaData(FactMetaData::valueTypeDouble) {
...
SimpleMissionItem::SimpleMissionItem(PlanMasterController *masterController, bool flyView, const MissionItem &missionItem)
    : VisualMissionItem(masterController, flyView), _missionItem(missionItem), _supportedCommandFact(0, "Command:", FactMetaData::valueTypeUint32), _altitudeFact(0, "Altitude", FactMetaData::valueTypeDouble),
      _amslAltAboveTerrainFact(0, "Alt above terrain", FactMetaData::valueTypeDouble), _param1MetaData(FactMetaData::valueTypeDouble), _param2MetaData(FactMetaData::valueTypeDouble), _param3MetaData(FactMetaData::valueTypeDouble),
      _param4MetaData(FactMetaData::valueTypeDouble), _param5MetaData(FactMetaData::valueTypeDouble), _param6MetaData(FactMetaData::valueTypeDouble), _param7MetaData(FactMetaData::valueTypeDouble) {
    _editorQml = QStringLiteral("qrc:/qml/SimpleItemEditor.qml");
...
QString SimpleMissionItem::abbreviation() const {
    ...
    switch (command()) {
        case MAV_CMD_NAV_TAKEOFF: return tr("Takeoff");
        case MAV_CMD_NAV_LAND: return tr("Land");
        case MAV_CMD_NAV_VTOL_TAKEOFF: return tr("Transition Direction");
        case MAV_CMD_NAV_VTOL_LAND: return tr("VTOL Land");
        case MAV_CMD_DO_SET_ROI:
        case MAV_CMD_DO_SET_ROI_LOCATION: return tr("ROI");
        case MAV_CMD_NAV_LOITER_TIME:
        case MAV_CMD_NAV_LOITER_TURNS:
        case MAV_CMD_NAV_LOITER_UNLIM:
        case MAV_CMD_NAV_LOITER_TO_ALT: return tr("Loiter");
...
bool SimpleMissionItem::showLoiterRadius() const { return specifiesCoordinate() && (_controllerVehicle->fixedWing() || _controllerVehicle->vtol()) && isLoiterItem(); }
    ...
    if (uiInfo && uiInfo->friendlyEdit()) {
        ...
        if (specifiesAltitude()) {
            ...
            switch (frame) {
                ...
                case MAV_FRAME_GLOBAL_TERRAIN_ALT: return true;
...
void SimpleMissionItem::_altitudeModeChanged(void) {
    switch (_altitudeMode) {
        case QGroundControlQmlGlobal::AltitudeModeTerrainFrame: _missionItem.setFrame(MAV_FRAME_GLOBAL_TERRAIN_ALT); break;
        case QGroundControlQmlGlobal::AltitudeModeCalcAboveTerrain:
            ...
            _amslAltAboveTerrainFact.setRawValue(qQNaN());
            break;
        case QGroundControlQmlGlobal::AltitudeModeAbsolute: _missionItem.setFrame(MAV_FRAME_GLOBAL); break;
        case QGroundControlQmlGlobal::AltitudeModeRelative: _missionItem.setFrame(MAV_FRAME_GLOBAL_RELATIVE_ALT); break;
        case QGroundControlQmlGlobal::AltitudeModeNone: qWarning() << "Internal Error SimpleMissionItem::_altitudeModeChanged: Invalid altitudeMode == AltitudeModeNone"; break;
        case QGroundControlQmlGlobal::AltitudeModeMixed: qWarning() << "Internal Error SimpleMissionItem::_altitudeModeChanged: Invalid altitudeMode == AltitudeModeMixed"; break;
    }
...
void SimpleMissionItem::_sendFriendlyEditAllowedChanged(void) { emit friendlyEditAllowedChanged(friendlyEditAllowed()); }
...
double SimpleMissionItem::specifiedGimbalYaw(void) { return _cameraSection->available() ? _cameraSection->specifiedGimbalYaw() : missionItem().specifiedGimbalYaw(); }

double SimpleMissionItem::specifiedGimbalPitch(void) { return _cameraSection->available() ? _cameraSection->specifiedGimbalPitch() : missionItem().specifiedGimbalPitch(); }

double SimpleMissionItem::specifiedVehicleYaw(void) { return command() == MAV_CMD_NAV_WAYPOINT ? missionItem().param4() : qQNaN(); }
...
int SimpleMissionItem::lastSequenceNumber(void) const { return sequenceNumber() + (_cameraSection ? _cameraSection->itemCount() : 0) + (_speedSection ? _speedSection->itemCount() : 0); }
    ...
    if (uiInfo && (uiInfo->specifiesCoordinate() || uiInfo->specifiesAltitudeOnly())) {
        switch (static_cast<MAV_CMD>(this->command())) {
            ...
            case MAV_CMD_NAV_VTOL_LAND:
                // Leave alone
                break;
            default: _altitudeFact.setRawValue(newAltitude); break;
        }
    }
}
...
double SimpleMissionItem::additionalTimeDelay(void) const {
    switch (command()) {
        ...
        case MAV_CMD_NAV_DELAY: return missionItem().param1();
        default: return 0;
    }
}
...
void SimpleMissionItem::_possibleAdditionalTimeDelayChanged(void) {
    switch (command()) {
        case MAV_CMD_NAV_WAYPOINT:
        case MAV_CMD_CONDITION_DELAY:
        case MAV_CMD_NAV_DELAY: emit additionalTimeDelayChanged(); break;
    }

    return;
}

bool SimpleMissionItem::isLandCommand(void) const { return MissionCommandTree::instance()->isLandCommand(static_cast<MAV_CMD>(this->command())); }
...
double SimpleMissionItem::amslEntryAlt(void) const {
    switch (_altitudeMode) {
        case QGroundControlQmlGlobal::AltitudeModeTerrainFrame: return _missionItem.param7() + _terrainAltitude;
        case QGroundControlQmlGlobal::AltitudeModeCalcAboveTerrain:
        case QGroundControlQmlGlobal::AltitudeModeAbsolute: return _missionItem.param7();
        case QGroundControlQmlGlobal::AltitudeModeRelative: return _missionItem.param7() + _masterController->missionController()->plannedHomePosition().altitude();
        case QGroundControlQmlGlobal::AltitudeModeNone: qWarning() << "Internal Error SimpleMissionItem::amslEntryAlt: Invalid altitudeMode:AltitudeModeNone"; return qQNaN();
        case QGroundControlQmlGlobal::AltitudeModeMixed: qWarning() << "Internal Error SimpleMissionItem::amslEntryAlt: Invalid altitudeMode:AltitudeModeMixed"; return qQNaN();
    }
