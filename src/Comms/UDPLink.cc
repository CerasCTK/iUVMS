...
bool UDPWorker::isConnected() const { return (_socket && _socket->isValid() && _isConnected); }
...
void UDPWorker::setupSocket() {
    ...
    (void)connect(_socket, &QUdpSocket::stateChanged, this, [this](QUdpSocket::SocketState state) {
        ...
        switch (state) {
            case QAbstractSocket::BoundState: _onSocketConnected(); break;
            case QAbstractSocket::ClosingState:
            case QAbstractSocket::UnconnectedState: _onSocketDisconnected(); break;
            default: break;
        }
    });
    ...
    if (UDPLinkLog().isDebugEnabled()) {
        ...
        (void)QObject::connect(_socket, &QUdpSocket::hostFound, this, []() { qCDebug(UDPLinkLog) << "UDP Host Found"; });
    }
}
...
void UDPWorker::_onSocketBytesWritten(qint64 bytes) { qCDebug(UDPLinkLog) << "Wrote" << bytes << "bytes"; }
...
void UDPWorker::_registerZeroconf(uint16_t port) {
    ...
    const DNSServiceErrorType result = DNSServiceRegister(&_dnssServiceRef, 0, 0, 0, regType, NULL, NULL, qToBigEndian(port), 0, NULL, &UDPWorker::_zeroconfRegisterCallback, this);
...
UDPLink::UDPLink(SharedLinkConfigurationPtr &config, QObject *parent) : LinkInterface(config, parent), _udpConfig(qobject_cast<const UDPConfiguration *>(config.get())), _worker(new UDPWorker(_udpConfig)), _workerThread(new QThread(this)) {
    // qCDebug(UDPLinkLog) << Q_FUNC_INFO << this;
...
bool UDPLink::isConnected() const { return _worker->isConnected(); }

bool UDPLink::_connect() { return QMetaObject::invokeMethod(_worker, "connectLink", Qt::QueuedConnection); }

void UDPLink::disconnect() { (void)QMetaObject::invokeMethod(_worker, "disconnectLink", Qt::QueuedConnection); }

void UDPLink::_onConnected() { emit connected(); }

void UDPLink::_onDisconnected() { emit disconnected(); }
...
void UDPLink::_onDataReceived(const QByteArray &data) { emit bytesReceived(this, data); }

void UDPLink::_onDataSent(const QByteArray &data) { emit bytesSent(this, data); }

void UDPLink::_writeBytes(const QByteArray &bytes) { (void)QMetaObject::invokeMethod(_worker, "writeData", Qt::QueuedConnection, Q_ARG(QByteArray, bytes)); }

bool UDPLink::isSecureConnection() { return QGCDeviceInfo::isNetworkWired(); }
