...
MockLinkMissionItemHandler::MockLinkMissionItemHandler(MockLink *mockLink, MAVLinkProtocol *mavlinkProtocol)
    : _mockLink(mockLink), _missionItemResponseTimer(nullptr), _failureMode(FailNone), _sendHomePositionOnEmptyList(false), _mavlinkProtocol(mavlinkProtocol), _failReadRequestListFirstResponse(true), _failReadRequest1FirstResponse(true),
...
bool MockLinkMissionItemHandler::handleMessage(const mavlink_message_t &msg) {
    switch (msg.msgid) {
        case MAVLINK_MSG_ID_MISSION_REQUEST_LIST: _handleMissionRequestList(msg); break;

        case MAVLINK_MSG_ID_MISSION_REQUEST_INT: _handleMissionRequest(msg); break;

        case MAVLINK_MSG_ID_MISSION_ITEM_INT: _handleMissionItem(msg); break;

        case MAVLINK_MSG_ID_MISSION_COUNT: _handleMissionCount(msg); break;
        ...
        case MAVLINK_MSG_ID_MISSION_CLEAR_ALL: _handleMissionClearAll(msg); break;
...
void MockLinkMissionItemHandler::_handleMissionClearAll(const mavlink_message_t &msg) {
    ...
    switch (_requestType) {
        case MAV_MISSION_TYPE_MISSION: _missionItems.clear(); break;
        case MAV_MISSION_TYPE_FENCE: _fenceItems.clear(); break;
        case MAV_MISSION_TYPE_RALLY: _rallyItems.clear(); break;
    ...
    if (_failureMode == FailReadRequestListNoResponse) {
        ...
        switch (_requestType) {
            case MAV_MISSION_TYPE_MISSION:
                ...
                break;
            case MAV_MISSION_TYPE_FENCE: itemCount = _fenceItems.count(); break;
            case MAV_MISSION_TYPE_RALLY: itemCount = _rallyItems.count(); break;
            default: Q_ASSERT(false);
        }
        ...
        mavlink_msg_mission_count_pack_chan(
            _mockLink->vehicleId(), MAV_COMP_ID_AUTOPILOT1, _mockLink->mavlinkChannel(),
...
void MockLinkMissionItemHandler::_handleMissionRequest(const mavlink_message_t &msg) {
    ...
    if (_failureMode == FailReadRequest0NoResponse && request.seq == 0) {
        ...
        if ((_failureMode == FailReadRequest0IncorrectSequence && request.seq == 0) || (_failureMode == FailReadRequest1IncorrectSequence && request.seq == 1)) {
        ...
        if ((_failureMode == FailReadRequest0ErrorAck && request.seq == 0) || (_failureMode == FailReadRequest1ErrorAck && request.seq == 1)) {
            ...
            switch (request.mission_type) {
                case MAV_MISSION_TYPE_MISSION:
                    ...
                    break;
                case MAV_MISSION_TYPE_FENCE: missionItemInt = _fenceItems[request.seq]; break;
                case MAV_MISSION_TYPE_RALLY: missionItemInt = _rallyItems[request.seq]; break;
                default: Q_ASSERT(false);
            }
            ...
            mavlink_msg_mission_item_int_pack_chan(
                ...
                missionItemInt.frame, missionItemInt.command, missionItemInt.current, missionItemInt.autocontinue, missionItemInt.param1, missionItemInt.param2, missionItemInt.param3, missionItemInt.param4, missionItemInt.x,
                missionItemInt.y, missionItemInt.z, _requestType
            );
            _mockLink->respondWithMavlinkMessage(responseMsg);
        }
    }
...
void MockLinkMissionItemHandler::_requestNextMissionItem(int sequenceNumber) {
    ...
    if (_failureMode == FailWriteRequest1NoResponse && sequenceNumber == 1) {
        ...
        if ((_failureMode == FailWriteRequest0IncorrectSequence && sequenceNumber == 0) || (_failureMode == FailWriteRequest1IncorrectSequence && sequenceNumber == 1)) {
            sequenceNumber++;
        }

        if ((_failureMode == FailWriteRequest0ErrorAck && sequenceNumber == 0) || (_failureMode == FailWriteRequest1ErrorAck && sequenceNumber == 1)) {
            qCDebug(MockLinkMissionItemHandlerLog) << "_requestNextMissionItem sending ack error due to failure mode";
            _sendAck(_failureAckResult);
        } else {
            mavlink_message_t message;

            mavlink_msg_mission_request_int_pack_chan(_mockLink->vehicleId(), MAV_COMP_ID_AUTOPILOT1, _mockLink->mavlinkChannel(), &message, _mavlinkProtocol->getSystemId(), _mavlinkProtocol->getComponentId(), sequenceNumber, _requestType);
...
void MockLinkMissionItemHandler::_sendAck(MAV_MISSION_RESULT ackType) {
    ...
    mavlink_msg_mission_ack_pack_chan(_mockLink->vehicleId(), MAV_COMP_ID_AUTOPILOT1, _mockLink->mavlinkChannel(), &message, _mavlinkProtocol->getSystemId(), _mavlinkProtocol->getComponentId(), ackType, _requestType, 0);
    _mockLink->respondWithMavlinkMessage(message);
}
...
void MockLinkMissionItemHandler::_handleMissionItem(const mavlink_message_t &msg) {
    ...
    switch (missionType) {
        case MAV_MISSION_TYPE_MISSION: _missionItems[seq] = missionItemInt; break;
        case MAV_MISSION_TYPE_FENCE: _fenceItems[seq] = missionItemInt; break;
        case MAV_MISSION_TYPE_RALLY: _rallyItems[seq] = missionItemInt; break;
        case MAV_MISSION_TYPE_ENUM_END:
        case MAV_MISSION_TYPE_ALL: qWarning() << "Internal error"; break;
    }
