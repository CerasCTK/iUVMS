...
MockLink::MockLink(SharedLinkConfigurationPtr &config)
    : LinkInterface(config), _missionItemHandler(this, MAVLinkProtocol::instance()), _name("MockLink"), _connected(false), _vehicleComponentId(MAV_COMP_ID_AUTOPILOT1), _inNSH(false), _mavlinkStarted(true),
      _mavBaseMode(MAV_MODE_FLAG_MANUAL_INPUT_ENABLED | MAV_MODE_FLAG_CUSTOM_MODE_ENABLED), _mavState(MAV_STATE_STANDBY), _firmwareType(MAV_AUTOPILOT_PX4), _vehicleType(MAV_TYPE_QUADROTOR), _vehicleAltitudeAMSL(_defaultVehicleHomeAltitude),
      _sendStatusText(false), _apmSendHomePositionOnEmptyList(false), _failureMode(MockConfiguration::FailNone), _sendHomePositionDelayCount(10) // No home position for 4 seconds
      ,
      _sendGPSPositionDelayCount(100) // No gps lock for 5 seconds
      ,
      _currentParamRequestListComponentIndex(-1), _currentParamRequestListParamIndex(-1), _logDownloadCurrentOffset(0), _logDownloadBytesRemaining(0), _adsbAngles{ 0 } {
...
bool MockLink::mavlinkAuxChannelIsSet(void) const { return (LinkManager::invalidMavlinkChannel() != _mavlinkAuxChannel); }
...
void MockLink::_sendHighLatency2(void) {
    ...
    mavlink_msg_high_latency2_pack_chan(
        ...
        (int32_t)(_vehicleLatitude * 1E7), (int32_t)(_vehicleLongitude * 1E7), (int16_t)_vehicleAltitudeAMSL,
...
void MockLink::_handleIncomingMavlinkMsg(const mavlink_message_t &msg) {
    ...
    switch (msg.msgid) {
        case MAVLINK_MSG_ID_HEARTBEAT: _handleHeartBeat(msg); break;
        case MAVLINK_MSG_ID_PARAM_REQUEST_LIST: _handleParamRequestList(msg); break;
        case MAVLINK_MSG_ID_SET_MODE: _handleSetMode(msg); break;
        case MAVLINK_MSG_ID_PARAM_SET: _handleParamSet(msg); break;
        case MAVLINK_MSG_ID_PARAM_REQUEST_READ: _handleParamRequestRead(msg); break;
        case MAVLINK_MSG_ID_FILE_TRANSFER_PROTOCOL: _handleFTP(msg); break;
        case MAVLINK_MSG_ID_COMMAND_LONG: _handleCommandLong(msg); break;
        case MAVLINK_MSG_ID_MANUAL_CONTROL: _handleManualControl(msg); break;
        case MAVLINK_MSG_ID_LOG_REQUEST_LIST: _handleLogRequestList(msg); break;
        case MAVLINK_MSG_ID_LOG_REQUEST_DATA: _handleLogRequestData(msg); break;
        case MAVLINK_MSG_ID_PARAM_MAP_RC: _handleParamMapRC(msg); break;
        default: break;
    }
}
...
void MockLink::_setParamFloatUnionIntoMap(int componentId, const QString &paramName, float paramFloat) {
    ...
    switch (paramType) {
        case MAV_PARAM_TYPE_REAL32: paramVariant = QVariant::fromValue(valueUnion.param_float); break;

        case MAV_PARAM_TYPE_UINT32: paramVariant = QVariant::fromValue(valueUnion.param_uint32); break;

        case MAV_PARAM_TYPE_INT32: paramVariant = QVariant::fromValue(valueUnion.param_int32); break;

        case MAV_PARAM_TYPE_UINT16: paramVariant = QVariant::fromValue(valueUnion.param_uint16); break;

        case MAV_PARAM_TYPE_INT16: paramVariant = QVariant::fromValue(valueUnion.param_int16); break;

        case MAV_PARAM_TYPE_UINT8: paramVariant = QVariant::fromValue(valueUnion.param_uint8); break;

        case MAV_PARAM_TYPE_INT8: paramVariant = QVariant::fromValue(valueUnion.param_int8); break;
...
void MockLink::_handleParamRequestRead(const mavlink_message_t &msg) {
    ...
    if (request.target_component == MAV_COMP_ID_ALL && paramName == "_HASH_CHECK") {
        ...
        mavlink_msg_param_value_pack_chan(_vehicleSystemId, componentId, mavlinkChannel(), &responseMsg, request.param_id, valueUnion.param_float, MAV_PARAM_TYPE_UINT32, 0, -1);
        respondWithMavlinkMessage(responseMsg);
        return;
    }
...
void MockLink::emitRemoteControlChannelRawChanged(int channel, uint16_t raw) {
    ...
    mavlink_msg_rc_channels_pack_chan(
        _vehicleSystemId, _vehicleComponentId, mavlinkChannel(),
        &responseMsg, // Outgoing message
...
void MockLink::_handleInProgressCommandLong(const mavlink_command_long_t &request) {
    ...
    mavlink_msg_command_ack_pack_chan(
        _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &commandAck, request.command, MAV_RESULT_IN_PROGRESS,
        1, // progress
        0, // result_param2
        0, // target_system
        0
    ...
    if (request.command != MockLink::MAV_CMD_MOCKLINK_RESULT_IN_PROGRESS_NO_ACK) {
        mavlink_msg_command_ack_pack_chan(
            _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &commandAck, request.command, commandResult,
            0, // progress
            0, // result_param2
            0, // target_system
            0
...
void MockLink::_handleCommandLong(const mavlink_message_t &msg) {
    ...
    switch (request.command) {
        ...
        case MAV_CMD_CONTROL_HIGH_LATENCY:
            ...
            break;
        case MAV_CMD_PREFLIGHT_STORAGE: commandResult = MAV_RESULT_ACCEPTED; break;
        ...
        case MockLink::MAV_CMD_MOCKLINK_RESULT_IN_PROGRESS_ACCEPTED:
        case MockLink::MAV_CMD_MOCKLINK_RESULT_IN_PROGRESS_FAILED:
        case MockLink::MAV_CMD_MOCKLINK_RESULT_IN_PROGRESS_NO_ACK: _handleInProgressCommandLong(request); return;
    }
...
void MockLink::sendUnexpectedCommandAck(MAV_CMD command, MAV_RESULT ackResult) {
    mavlink_message_t commandAck;
    mavlink_msg_command_ack_pack_chan(
        _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &commandAck, command, ackResult,
        0, // progress
        0, // result_param2
        0, // target_system
        0
...
void MockLink::setMissionItemFailureMode(MockLinkMissionItemHandler::FailureMode_t failureMode, MAV_MISSION_RESULT failureAckResult) { _missionItemHandler.setFailureMode(failureMode, failureAckResult); }
...
void MockLink::_sendHomePosition(void) {
    ...
    mavlink_msg_home_position_pack_chan(
        _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &msg, (int32_t)(_vehicleLatitude * 1E7), (int32_t)(_vehicleLongitude * 1E7), (int32_t)(_defaultVehicleHomeAltitude * 1000), 0.0f, 0.0f, 0.0f, &bogus[0], 0.0f, 0.0f, 0.0f, 0
    );
    respondWithMavlinkMessage(msg);
}
...
void MockLink::_sendGpsRawInt(void) {
    ...
    mavlink_msg_gps_raw_int_pack_chan(
        ...
        GPS_FIX_TYPE_3D_FIX, (int32_t)(_vehicleLatitude * 1E7), (int32_t)(_vehicleLongitude * 1E7), (int32_t)(_vehicleAltitudeAMSL * 1000),
        3 * 100,    // hdop
        3 * 100,    // vdop
...
void MockLink::_sendGlobalPositionInt(void) {
    ...
    mavlink_msg_global_position_int_pack_chan(
        _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &msg,
        timeTick++,                                                                                                                                                                                     // time since boot
        (int32_t)(_vehicleLatitude * 1E7), (int32_t)(_vehicleLongitude * 1E7), (int32_t)(_vehicleAltitudeAMSL * 1000), (int32_t)((_vehicleAltitudeAMSL - _defaultVehicleHomeAltitude) * 1000), 0, 0, 0, // no speed sent
        UINT16_MAX
    ); // no heading sent
    respondWithMavlinkMessage(msg);
}
...
void MockLink::_sendExtendedSysState(void) {
    ...
    mavlink_msg_extended_sys_state_pack_chan(
        _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &msg, MAV_VTOL_STATE_UNDEFINED, _vehicleAltitudeAMSL > _defaultVehicleHomeAltitude ? MAV_LANDED_STATE_IN_AIR : MAV_LANDED_STATE_ON_GROUND
    );
    respondWithMavlinkMessage(msg);
}
...
void MockLink::_sendChunkedStatusText(uint16_t chunkId, bool missingChunks) {
    ...
    for (int i = 0; i < cChunks; i++) {
        ...
        mavlink_msg_statustext_pack_chan(_vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &msg, MAV_SEVERITY_INFO, msgBuf, chunkId,
...
void MockLink::_sendStatusTextMessages(void) {
    ...
    static const struct StatusMessage rgMessages[] = {
        { MAV_SEVERITY_INFO, "#Testing audio output" },    { MAV_SEVERITY_EMERGENCY, "Status text emergency" }, { MAV_SEVERITY_ALERT, "Status text alert" },
        { MAV_SEVERITY_CRITICAL, "Status text critical" }, { MAV_SEVERITY_ERROR, "Status text error" },         { MAV_SEVERITY_WARNING, "Status text warning" },
        { MAV_SEVERITY_NOTICE, "Status text notice" },     { MAV_SEVERITY_INFO, "Status text info" },           { MAV_SEVERITY_DEBUG, "Status text debug" },
    };
    ...
    for (size_t i = 0; i < sizeof(rgMessages) / sizeof(rgMessages[0]); i++) {
        ...
        mavlink_msg_statustext_pack_chan(
            _vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &msg, status->severity, status->msg,
...
MockConfiguration::MockConfiguration(const QString &name) : LinkConfiguration(name) {}

MockConfiguration::MockConfiguration(const MockConfiguration *source) : LinkConfiguration(source) {
...
MockLink *MockLink::startPX4MockLink(bool sendStatusText, MockConfiguration::FailureMode_t failureMode) { return _startMockLinkWorker("PX4 MultiRotor MockLink", MAV_AUTOPILOT_PX4, MAV_TYPE_QUADROTOR, sendStatusText, failureMode); }

MockLink *MockLink::startGenericMockLink(bool sendStatusText, MockConfiguration::FailureMode_t failureMode) { return _startMockLinkWorker("Generic MockLink", MAV_AUTOPILOT_GENERIC, MAV_TYPE_QUADROTOR, sendStatusText, failureMode); }
...
void MockLink::_handlePreFlightCalibration(const mavlink_command_long_t &request) {
    ...
    mavlink_message_t msg;
    mavlink_msg_statustext_pack_chan(_vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &msg, MAV_SEVERITY_INFO, pCalMessage, 0, 0); // Not chunked
    respondWithMavlinkMessage(msg);
}
...
void MockLink::_logDownloadWorker(void) {
    if (_logDownloadBytesRemaining != 0) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            mavlink_message_t responseMsg;
            mavlink_msg_log_data_pack_chan(_vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &responseMsg, _logDownloadLogId, _logDownloadCurrentOffset, bytesToRead, &buffer[0]);
            respondWithMavlinkMessage(responseMsg);
...
void MockLink::_sendADSBVehicles(void) {
    for (int i = 0; i < _adsbVehicles.size(); ++i) {
        ...
        mavlink_msg_adsb_vehicle_pack_chan(
            ...
            _adsbVehicles[i].coordinate.latitude() * 1e7, _adsbVehicles[i].coordinate.longitude() * 1e7, ADSB_ALTITUDE_TYPE_GEOMETRIC,
...
bool MockLink::_handleRequestMessage(const mavlink_command_long_t &request, bool &noAck) {
    ...
    switch ((int)request.param1) {
        case MAVLINK_MSG_ID_AUTOPILOT_VERSION: {
            switch (_failureMode) {
                case MockConfiguration::FailNone: break;
                case MockConfiguration::FailInitialConnectRequestMessageAutopilotVersionFailure: return false;
                case MockConfiguration::FailInitialConnectRequestMessageAutopilotVersionLost: return true;
                default: break;
            }
        ...
        case MAVLINK_MSG_ID_PROTOCOL_VERSION: {
            switch (_failureMode) {
                case MockConfiguration::FailNone: break;
                case MockConfiguration::FailInitialConnectRequestMessageProtocolVersionFailure: return false;
                case MockConfiguration::FailInitialConnectRequestMessageProtocolVersionLost: return true;
                default: break;
            }

            uint8_t nullHash[8] = { 0 };
            mavlink_message_t responseMsg;
            mavlink_msg_protocol_version_pack_chan(_vehicleSystemId, _vehicleComponentId, mavlinkChannel(), &responseMsg, 200, 100, 200, nullHash, nullHash);
            respondWithMavlinkMessage(responseMsg);
        }
            return true;
        ...
        case MAVLINK_MSG_ID_DEBUG:
            switch (_requestMessageFailureMode) {
                case FailRequestMessageNone: break;
                case FailRequestMessageCommandAcceptedMsgNotSent: return true;
                case FailRequestMessageCommandUnsupported: return false;
                case FailRequestMessageCommandNoResponse: noAck = true; return true;
            }
            {
                mavlink_message_t responseMsg;
...
void MockLink::_sendRemoteIDArmStatus() {
    ...
    mavlink_msg_open_drone_id_arm_status_pack(_vehicleSystemId, MAV_COMP_ID_ODID_TXRX_1, &msg, MAV_ODID_ARM_STATUS_GOOD_TO_ARM, err.toStdString().c_str());
    respondWithMavlinkMessage(msg);
}
